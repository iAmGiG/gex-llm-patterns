...
class PriceTokenizer:
    ...
    def tokenize_price_series(self, 
                             prices,
                             return_window = 1) :
        """
    ...
    def tokenize_with_volume(self, 
        ...
        for i, (price_token, volume_token) in enumerate(zip(price_tokens, volume_tokens)):
            result.append({
                'price': price_token,
                'volume': volume_token,
                'composite': f"{price_token}_{volume_token}"
            })
        
        return result
    
    def tokenize_price_levels(self, 
                             price,
                             support= None,
                             resistance= None,
                             vwap= None) :
    ...
    def validate_price_tokens(self, 
                             prices,
                             tokens) :
        """
        ...
        validation = {
            'length_match': len(prices) == len(tokens),
            'return_stats': self.get_return_statistics(prices)
        }
        ...
        validation['token_distribution'] = {
            k: v/len(tokens) for k, v in token_counts.items() if len(tokens) > 0
        }
        ...
        if len(returns.dropna()) > 0:
            ...
            if extreme_up.any():
                ...
                validation['extreme_up_accuracy'] = (
                    sum(1 for t in extreme_up_tokens if 'MOON' in t) / 
                    len(extreme_up_tokens) if extreme_up_tokens else 0
                )
            
            if extreme_down.any():
                extreme_down_tokens = [tokens[i] for i, v in enumerate(extreme_down) if v]
                validation['extreme_down_accuracy'] = (
                    sum(1 for t in extreme_down_tokens if 'CRASH' in t) / 
                    len(extreme_down_tokens) if extreme_down_tokens else 0
                )
