...
class EventTokenizer:
    ...
    def detect_volatility_events(self, 
                                 vix= None,
                                 returns= None) :
        """
            ...
            for date in realized_vol[regime_changes].index:
                if not pd.isna(realized_vol[date]):
                    events.append({
                        ...
                        'details': {
                            'realized_vol': realized_vol[date],
                            'expected': vol_ma[date]
                        }
                    })
    ...
    def validate_event_detection(self,
        ...
        if known_events:
            ...
            if validation['precision'] + validation['recall'] > 0:
                validation['f1_score'] = (
                    2 * validation['precision'] * validation['recall'] / 
