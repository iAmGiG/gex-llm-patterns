...
\begin{itemize}
\item \texttt{01\_Introduction.tex}: paragraphs 1--4 of \S{}~1 fully rewritten; \S{}~1.1 Research Questions, \S{}~1.2 Contributions, \S{}~1.4 Positioning, \S{}~1.5 Paper Organization retained unchanged from the prior revision commits.
...
in new \S{}~3.9 "Markov-Switching Benchmark" and new \S{}~4.7 "Comparison
...
(R3.5d) above. \S{}~6 Conclusion contribution 3 now describes the 0DTE
correspondence as "coincides with" rather than "drove"; \S{}~5.3 softens
...
Deeper \S{}~5.3 revision is still scheduled in C2 below.

\textbf{Change location:}

\begin{itemize}
\item \S{}~3.9 Markov-Switching Benchmark (new subsection)
\item \S{}~4.7 Comparison with Markov-Switching Benchmark (new subsection, Table 6, Figure 8)
...
\item \S{}~6 Conclusion + \S{}~5.3 + \S{}~5.7 moderations as described under R3.5d
\end{itemize}

\textbf{Status:} (a) done; (b) done (moderations in \S{}~5.3 applied in B4 plus a
fuller \S{}~5.3 rewrite in the C2 commit). \S{}~5.3 now explicitly (i) frames
...
\textbf{(c) Temperature and reproducibility.} Appendix A also contains a
...
been added as new \S{}~4.6 "Threshold Sensitivity" with Figure 7
...
Key findings reported in \S{}~4.6:
...
\begin{itemize}
\item New Appendix A on pp. 20--25 (parts (a) and (c) above).
...
\item New \S{}~4.6 "Threshold Sensitivity" subsection with Figure 7 (part (b)).
\end{itemize}
...
\begin{itemize}
\item \S{}~4.4 Phase 4 (2020 vs 2024, 223 each): Pearson's $\chi$$^{2}$ = 213.67 (df=1, p = 2.2$\times$10$^{-}$$^{4}$$^{8}$), Yates-corrected $\chi$$^{2}$ = 210.90 (p = 8.7$\times$10$^{-}$$^{4}$$^{8}$), Fisher's exact two-sided p = 1.8$\times$10$^{-}$$^{5}$$^{2}$ with odds ratio 31.3, $\varphi$ = 0.69 (refined from the previously rounded 0.672), and a risk difference of 69.1pp with a 95\% Wald CI of [62.4, 75.7]pp.
\item \S{}~4.5 Phase 5 (2023$\rightarrow$2024 transition, 228 vs 241): $\chi$$^{2}$ = 314.4 (p = 2.4$\times$10$^{-}$$^{7}$$^{0}$), Fisher's exact p = 9.9$\times$10$^{-}$$^{8}$$^{7}$ (OR diverges because all 241 2024 windows are detected), $\varphi$ = 0.82 (refined from 0.783).
...
\begin{itemize}
\item \S{}~6 Conclusion contribution 2 now reports the 69.1pp separation with explicit CI brackets on each side and Fisher's exact p, and cites the 45-configuration robustness of the 50pp gap, rather than citing the separation as a standalone point estimate.
\item \S{}~6 Conclusion contribution 3 replaces "0DTE-driven structural reorganization" with language that identifies temporal coincidence and explicitly acknowledges alternative contemporaneous factors (interest rates, passive flow concentration, market-maker inventory), noting that stronger causal evidence would require a natural experiment.
\item \S{}~5.3 "Market Structure Evolution" similarly softens the "tipping-point dynamic strengthens the structural interpretation" phrasing to "is consistent with, rather than proof of" and cross-references \S{}~5.7 Limitations for the causal-identification caveat.
\end{itemize}
...
\begin{itemize}
\item \texttt{04\_Results.tex} \S{}~4.1 new "Statistical conventions" paragraph
\item \texttt{04\_Results.tex} Phase 1/3 inline rates in text
...
window/threshold robustness in \S{}~4.6, moderated claim language in \S{}~5.3
and \S{}~6) landed across the B1, B2, B3, B4, C2 revision commits.
...
the practitioner side. The previous \S{}~5.6 "Practitioner Implications"
...
\textbf{Change location:} \S{}~5.6 "Practical Implications" (renamed from
...
\textbf{Change location:} \S{}~5.7 Limitations and Future Work (p.\textbackslash{} 17 in the
...
opener and \S{}~5.5 Dispersed Knowledge were the two heaviest) and rewrote
...
\textbf{Change location:} targeted tightening in \S{}~5.5 Dispersed Knowledge
(sentences broken up); \S{}~1 opener and \S{}~5.3 Market Structure Evolution
