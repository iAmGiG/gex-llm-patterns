...
\begin{itemize}
    ...
    \item \textbf{Maximum completion tokens:} not set; the OpenAI
          ...
          1{,}412 windows ranged from 2{,}457 to 5{,}924 tokens, so
    ...
    \item \textbf{Access mode:} OpenAI Batch API,
          ...
          completion window. Submissions are sized per validation
          phase rather than at a fixed per-batch count.
\end{itemize}
...
parameter; we did not set one.) Reproducibility at the \emph{distributional}
...
(the 1{,}412 real-window detections retained in the published results
cache plus the 809 synthetic-control evaluations), the schema-validation
failure rate was 0\%---every retained response parsed cleanly into the
output schema.
