...
Concretely, the row \texttt{SPY, 2024-03-15: Net GEX: -\$32.9B, Flip: \$485.00} becomes \texttt{Day T+0, INDEX\_1: Net GEX: -\$32.9B, Flip: \$485.00}: quantitative values are unchanged, only the contextual identifiers change. A model memorising ``January 2024 SPY negative gamma'' cannot recover that association from the obfuscated form, so detection must come from numerical structure alone.
