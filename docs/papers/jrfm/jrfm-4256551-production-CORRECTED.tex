...
\pubyear{2026}
\copyrightyear{2026}
\externaleditor{Shigeyuki Hamori} % More than 1 editor, please add `` and '' before the last editor name
\datereceived{29 March 2026}
\daterevised{2 May 2026} % Comment out if no revised date
\dateaccepted{14 May 2026}
\datepublished{21 May 2026}
...
% Authors
\Author{{Christopher Regan} %MDPI: This name is different from the one submitted online at susy.mdpi.com. Please confirm which is correct.
 *\orcidA{} and {Ying Xie} *%MDPI: Based on our previous email correspondence, I have added Dr. Ying Xie as a corresponding author. Please confirm.
\orcidB{}}
 %MDPI: Dear Authors,
...
\textbf{Single-day validation.} Applied to 242 trading days ({SPY, 2024}), %MDPI: Is this a reference citation? if so, please add detailed information about this reference to your list of references.
...
Market makers face regulatory requirements to maintain delta-neutral positions, creating systematic hedging flows when gamma exposure accumulates. \citet{grossman1988liquidity} %\citeyear{grossman1988liquidity} %Please change the & to and.
...
{This} %MDPI: We added indention here. Please confirm.
...
The core innovation is temporal obfuscation---systematically stripping identifying context while preserving structural market mechanics. Five transformations are applied: (i) absolute dates {(``16 January 2024'')} %MDPI: We changed date to standard format. Please confirm this revision. The following highlights are the same.
...
Concretely, the row \texttt{{SPY,} %MDPI: Please confirm if the font is necessary, if not, please remove it. Please review and confirm throughout the manuscript.
{15 March 2024}: Net GEX: $-$\$32.9B, Flip: \$485.00} becomes \texttt{Day T + 0, INDEX\_1: Net GEX: $-$\$32.9B, Flip: \$485.00}: quantitative values are unchanged, only the contextual identifiers change. A model memorizing ``January 2024 SPY negative gamma'' cannot recover that association from the obfuscated form; so, detection must come from the numerical structure alone.
...
\begin{figure}[H]
%\centering
\includegraphics[width=0.9\textwidth]{figures/fig02_regime_window.png}
...
{Expected:} %MDPI: We added indention here. Please confirm.
...
\textls[-5]{\textbf{Phase 4} (2020 Comparison, 223 windows): Test pre-0DTE era (2020, $<$5\% 0DTE volume) against post-0DTE (2024, $\approx$46\% SPY).}
...
\textls[-15]{We use OpenAI o4-mini \citep{openai2024reasoning}, a reasoning model that runs at a fixed internal sampling temperature; the request supplies neither a temperature, a \texttt{max\_completion\_tokens},} nor a \texttt{seed} parameter, so OpenAI defaults apply. Evaluations are processed via the OpenAI Batch API (\texttt{/v1/chat/completions}, asynchronous 24-h completion window, 100\% completion rate). Each request is a single user message containing the role instruction (``financial market analyst identifying persistent dealer gamma regimes''), the 30-day obfuscated GEX sequence, and the classification criteria; the prompt asks the model to return JSON with regime type, confidence (0--100), reasoning trace, and computed metrics. The total processing cost across all 2221 evaluations was \$11.07. The complete prompt, API configuration, and output schema are reproduced verbatim in Appendix~\ref{app:prompt}.
...
Applied to 242 trading days {(SPY, 2024)}, %MDPI: Is this a reference citation? if so, please add detailed information about this reference to your list of references.
...
\midrule
\multicolumn{2}{l}{\textit{Detection comparison}}\\ %MDPI: Please add an explanation for the use of italics in the table footer. If the italics is unnecessary, please remove it. The following highlights are the same.
...
\paragraph{{Statistical conventions used in this section:}} %MDPI: Please confirm if it should be formatted as list or as title.
...
\begin{figure}[H]
%\centering
\includegraphics[width=0.85\textwidth]{figures/fig03_validation_pipeline.png}
\caption{Multi-phase validation pipeline. Each box reports the phase name, the number of 30-day windows evaluated, and the detection rate with 95\% bootstrap confidence interval. Phase~1 (Q1 2024 baseline) anchors the framework at 71.2\% detection; Phase~2 synthetic controls (shuffle/transitional/ low-magnitude) test false-positive discipline; Phase~3 (full 2024) confirms 81.2\% generalization; Phase~4 (full 2020) is the pre-0DTE baseline at 12.1\%; Phase~5 extends to a 2020--2025 panel. \textbf{{Read this figure as}%MDPI: Please confirm if the bold formatting is necessary; if not, please remove it.
...
Pearson's $\chi^2 = 213.67$ (df $= 1$, $p = 2.2 \times 10^{-48}$; Yates-corrected $\chi^2 = 210.90$, \mbox{$p = 8.7 \times 10^{-48}$)},
...
\begin{figure}[H]
%\centering
\includegraphics[width=0.9\textwidth]{figures/fig04_selectivity.png}
\caption{{Framework} %MDPI: 1. Please cite this figure in the text and ensure that the first citation of each figure appears in numerical order. 2. Please confirm if the explanation of the colors needs to be added in the figure caption.
...
\midrule
\textbf{{Total} %MDPI: Please confirm if the bold formatting is necessary; if not, please remove it. The following highlights are the same.
} & \textbf{{1412}} & \textbf{{646}} & \textbf{{45.8\%}} & \textbf{{[43.2, 48.4]\%}} & \textbf{{--}} & \textbf{{--}} \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{figure}[H]
%\centering
\includegraphics[width=0.99\textwidth]{figures/fig05_gex_magnitude_distribution.png}
\caption{{Distribution} %MDPI: Please cite this figure in the text and ensure that the first citation of each figure appears in numerical order.
 of 30-day window average absolute GEX magnitude for 2020 (blue) and 2024 (orange), with the \$5B classification threshold shown as a dashed vertical line. The 2020 distribution (mean \$3.0B) lies almost entirely below the threshold; the 2024 distribution (mean \$20.3B) lies almost entirely above it. \textbf{{Read this figure as follows:}} the magnitude criterion alone---before persistence or stability are even checked---already separates the two eras, and the chosen \$5B threshold is positioned in the trough between the two distributions rather than in the bulk of either.}
...
windows do not change the point estimates.} under a 5~$\times$~3~$\times$~3~grid of alternative threshold combinations (persistence~$\in \{60, 65, 70, 75, 80\}$\%, magnitude~$\in~\{\$3\text{B}, \$5\text{B}, \$7\text{B}\}$, flips~$\leq~\{3, 5, 7\}$; 45 configurations in total). The full sweep was
produced deterministically by
...
{Figure~\ref{fig:threshold_sensitivity}} %MDPI: Wrong Figures citation order: Figure 6, previous number is Figure 3. Figures should be mentioned in numerical order. Please confirm and revise.
...
\begin{figure}[H]
%\centering
\includegraphics[width=0.99\textwidth]{figures/fig09_threshold_sensitivity.png}
\caption{{Threshold} %MDPI: Please change the hyphen (-) between numbers into an en dash (−, “U+2013”), e.g., “2019-2022” should be “2019–2022” and “0-9” should be “0–9”.
...
Markov-switching benchmark described in {Section} %MDPI: We changed symbol to Section. Please confirm this revision.
...
\begin{figure}[H]
%\centering
\includegraphics[width=0.99\textwidth]{figures/fig10_hmm_agreement.png}
\caption{{Markov-switching} %MDPI: We revised the hyphen in the figure into minus sign. Please confirm. Please cite this figure in the text and ensure that the first citation of each figure appears in numerical order.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.99\textwidth]{figures/fig06_detection_progression.png}
\caption{{Per-year} %MDPI: Please change the hyphen (-) between numbers into an en dash (−, “U+2013”), e.g., “2019-2022” should be “2019–2022” and “0-9” should be “0–9”.
 regime detection rate (bars) and average absolute GEX magnitude (line) for SPY 2020--2025. Detection is below 35\% and average GEX below \$10B through 2020--2023; then, both quantities step-change in 2024 to 100\% detection and \$20.3B average GEX and remain there in 2025. The 2022--2023 non-monotonic dip (32.4\%~$\rightarrow$~20.2\%) coincides with the 2023 FOMC/banking-stress volatility episode. \textbf{{Read this figure as follows:}} the LLM regime-detection signal is not a smooth secular trend but a discrete step-change, coincident with the maturation of the 0DTE options market; it is not a proof of causation but is less easily reconciled with gradual drift.}
...
\item \textbf{{Single-asset scope}}: %MDPI: We formatted the following cotntent as a list and added label. Please confirm.
...
than as a demonstrated causal channel (see {Section} \ref{sec:discussion:0dte}).
...
(see {Section} \ref{sec:regime} sensitivity analysis), but the chosen thresholds
...
\noindent\textbf{{Future directions}}: %MDPI: please confirm if this should be list or need add indention here.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\institutionalreview{{Not applicable.}}%MDPI:We re the highlighted sentence {This study uses publicly available market data and does not involve human participants.}; generally, for cases where it does not apply, simply writing "Not applicable" is sufficient.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\informedconsent{Not applicable.}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\dataavailability{All code, data preprocessing scripts, and validation results are publicly available at \url{https://github.com/iAmGiG/gex-llm-patterns} (accessed on 24 May 2026). %MDPI: We added the access date.
 Raw options data was obtained from Alpha Vantage under academic API access.}


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\acknowledgments{We acknowledge the computational resources provided by the College of Computing and Software Engineering at Kennesaw State University. This research was conducted as part of the first author's doctoral dissertation. We thank Alpha Vantage for academic API access to historical options market data. During the preparation of this manuscript, the authors used Anthropic's Claude Code (\url{https://www.anthropic.com/claude-code}; accessed on 24 May 2026), a command-line AI coding assistant, with Anthropic's Claude Opus 4 model family (versions 4.5--4.7, used between May 2025 and May 2026) for the purposes of clarifying and refining the written presentation. The authors have reviewed and edited the output and take full responsibility for the content of this publication.}
...
\begin{itemize}
    ...
    \item \textbf{Access mode:} OpenAI Batch API,
          \texttt{/v1/chat/completions} endpoint, 24-h asynchronous
          completion window. Submissions are sized per validation phase
          rather than at a fixed per-batch count.
\end{itemize}

\noindent\textbf{{Reproducibility note.}} %MDPI: Please confirm if the indention should be added here.
...
(i)~the large sample size ({N} %MDPI: Please confirm if “n” should appear in italics as variables. If yes, please change it in the whole text.
...
ticker symbol is absent. No other identifying context is {supplied.}%MDPI: Please confirm if the blank row should be removed in the verbatim. if yes, please please change it in the whole text.
...
\begin{itemize}
    ...
    \item \texttt{regime\_type} (string)---one of
          \texttt{persistent\_positive}, \texttt{persistent\_negative}, \linebreak
...
\noindent
{Parsing} %MDPI: Please confirm if the indention should be added here.
...
\newblock {\em Journal of International Money and Finance\/},~{\em 124}, 102627. [\href{http://dx.doi.org/10.1016/j.jimonfin.2022.102627}{CrossRef}]

\bibitem[\protect\citeauthoryear{Ang \BBA\ Bekaert}{Ang \BBA\
Bekaert}{2002}]{ang2002regime}
Ang, A., \BBA\  Bekaert, G. (2002).
\newblock International asset allocation with regime shifts.
\newblock {\em Review of Financial Studies\/},~{\em 15\/}(4), 1137--1187. [\href{http://dx.doi.org/10.1093/rfs/15.4.1137}{CrossRef}]

\bibitem[\protect\citeauthoryear{Black \& Scholes}{Black \&
Scholes}{1973}]{black1973pricing}
Black, F., \& Scholes, M. (1973).
\newblock The pricing of options and corporate liabilities.
\newblock {\em Journal of Political Economy\/},~{\em 81\/}(3), 637--654. [\href{http://dx.doi.org/10.1086/260062}{CrossRef}] [\href{http://www.ncbi.nlm.nih.gov/pubmed/24104663}{PubMed}]
...
\newblock {\em Statistical Science\/},~{\em 16\/}(2), 101--133. [\href{http://dx.doi.org/10.1214/ss/1009213286}{CrossRef}]
...
activity} {(CBOE Research Report).}  CBOE Insights.
...
\newblock {0DTEs: Trading, gamma risk and volatility propagation} (SSRN Working Paper).
\newblock {SSRN ID~4692190.}  SSRN.
 [\href{http://dx.doi.org/10.2139/ssrn.4692190}{CrossRef}]
...
ACL 2024} (pp.~12039--12050). Association for
...
\newblock {\em Review of Financial Studies\/},~{\em 22\/}(10), 4259--4299. [\href{http://dx.doi.org/10.1093/rfs/hhp005}{CrossRef}]

\bibitem[\protect\citeauthoryear{Grossman \BBA\ Miller}{Grossman \BBA\
Miller}{1988}]{grossman1988liquidity}
Grossman, S.~J., \BBA\ Miller, M. H. (1988).
\newblock Liquidity and market structure.
\newblock {\em Journal of Finance\/},~{\em 43\/}(3), 617--633. [\href{http://dx.doi.org/10.1111/j.1540-6261.1988.tb04594.x}{CrossRef}]
...
\newblock {\em Econometrica\/},~{\em 57\/}(2), 357--384. [\href{http://dx.doi.org/10.2307/1912559}{CrossRef}]
...
\bibitem[\protect\citeauthoryear{Lopez-Lira \BBA\ Tang}{Lopez-Lira \BBA\ Tang}{2023}]{lopez2023can}
Lopez-Lira, A., \BBA\ Tang, Y. (2023).
\newblock Can {ChatGPT} forecast stock price movements? {Return} predictability
and large language models.
\newblock \textit{arXiv},  arXiv:2304.07619. [\href{http://dx.doi.org/10.2139/ssrn.4412788}{CrossRef}]
...
\newblock \textit{arXiv},  arXiv:2504.14765. [\href{http://dx.doi.org/10.48550/arXiv.2504.14765}{CrossRef}]

\bibitem[\protect\citeauthoryear{Marcus \BBA\ Davis}{Marcus \BBA\
Davis}{2019}]{marcus2019rebooting}
Marcus, G., \BBA\ Davis, E. (2019).
...
121\/}(41), e2322420121. [\href{http://dx.doi.org/10.1073/pnas.2322420121}{CrossRef}] [\href{http://www.ncbi.nlm.nih.gov/pubmed/39365822}{PubMed}]
...
\newblock {\em Journal of Financial Economics\/},~{\em 78\/}(1), 49--87. [\href{http://dx.doi.org/10.2139/ssrn.519044}{CrossRef}]
...
\newblock {\em Quantitative Finance\/},~{\em 18\/}(1), 83--95. [\href{http://dx.doi.org/10.1080/14697688.2017.1342857}{CrossRef}]
...
\bibitem[\protect\citeauthoryear{Regan \& Xie}{Regan \&
...
Macau, China, December 8--11, 2025}. {IEEE.} arXiv:2512.17923.
...
computational linguistics} (pp.\  4902--4912).  {Association for Computational Linguistics.}
\newblock [\href{http://dx.doi.org/10.18653/v1/2020.acl-main.442}{CrossRef}]

\bibitem[\protect\citeauthoryear{{SpotGamma}}{{SpotGamma}}{2021}]{spotgamma2021}
{SpotGamma}. (2021).
\newblock \textit{Understanding gamma exposure} {(Technical Documentation).}  Available online:  \url{https://spotgamma.com/gamma-exposure-gex/} (accessed on 28 March 2026).
