...
Obfuscation testing removes all temporal context while preserving structural data (e.g., ``Day T+0: Net GEX: -\$32.9B''). The 71.5\% detection rate without dates, ticker symbols, or market context strongly suggests causal reasoning rather than temporal association.
...
Results indicate significant Granger causality at lag 2 (p = 0.042), confirming gamma exposure contains predictive information for T+2 forward volatility. Robustness checks across six specifications validate this finding (strongest: p = 0.021). Within the negative GEX regime, volatility does not correlate linearly with GEX magnitude (Pearson r = -0.01, p = 0.85), confirming a threshold effect where negative gamma presence drives volatility, not its specific level.
...
This persistent negative gamma regime, driven by 0DTE options growth \cite{cboe2023,jpmorgan2023}, represents a structural shift from historical regime-switching dynamics \cite{ni2005stock,garleanu2009demand}. The uniform environment provides a rigorous test: the LLM must identify underlying constraints without exploiting regime variation.
