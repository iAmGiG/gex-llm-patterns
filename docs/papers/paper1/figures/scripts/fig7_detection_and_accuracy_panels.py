...
# Plot bars
bars1 = ax.bar(x - width/2, biased_detection, width,
               label='Biased Prompts (with regime hints)',
               color='#2E86AB', alpha=0.9, edgecolor='black', linewidth=1.5)
bars2 = ax.bar(x + width/2, unbiased_detection, width,
...
ax.set_title('Impact of Prompt Bias on Detection Rates',
...
for i in range(len(pattern_labels)):
    ...
    ax.text(x[i], y_pos, f'Δ={delta:.1f}%', ha='center', va='bottom',
...
bars1 = ax1.bar(x - width/2, biased_detection, width, label='Biased',
                color='#2E86AB', alpha=0.9, edgecolor='black', linewidth=1.5)
bars2 = ax1.bar(x + width/2, unbiased_detection, width, label='Unbiased',
                color='#F77F00', alpha=0.9, edgecolor='black', linewidth=1.5)

ax1.axhline(y=60, color='red', linestyle='--', linewidth=2,
            label='Threshold', alpha=0.7, zorder=0)

ax1.set_xlabel('Pattern Type', fontsize=12, fontweight='bold')
ax1.set_ylabel('Detection Rate (%)', fontsize=12, fontweight='bold')
ax1.set_title('(a) Detection Rates', fontsize=13, fontweight='bold')
ax1.set_xticks(x)
ax1.set_xticklabels(pattern_labels, fontsize=11)
ax1.set_ylim(0, 110)
# Changed to 'lower right' to match right panel consistency
...
for bar1, bar2 in zip(bars1, bars2):
    ...
    ax1.text(bar1.get_x() + bar1.get_width()/2., h1 + 1,
             f'{h1:.0f}%', ha='center', va='bottom',
             fontsize=10, fontweight='bold')
    ax1.text(bar2.get_x() + bar2.get_width()/2., h2 + 3,
             f'{h2:.1f}%', ha='center', va='bottom',
             fontsize=10, fontweight='bold')

# RIGHT PANEL: Prediction Accuracy
bars3 = ax2.bar(x - width/2, biased_accuracy, width, label='Biased',
                color='#2E86AB', alpha=0.9, edgecolor='black', linewidth=1.5)
bars4 = ax2.bar(x + width/2, unbiased_accuracy, width, label='Unbiased',
                color='#F77F00', alpha=0.9, edgecolor='black', linewidth=1.5)

ax2.axhline(y=90, color='green', linestyle='--', linewidth=2,
            label='High Accuracy', alpha=0.7, zorder=0)

ax2.set_xlabel('Pattern Type', fontsize=12, fontweight='bold')
...
ax2.set_xticks(x)
ax2.set_xticklabels(pattern_labels, fontsize=11)
ax2.set_ylim(85, 100)
...
for bar3, bar4 in zip(bars3, bars4):
    h3 = bar3.get_height()
    h4 = bar4.get_height()
    ax2.text(bar3.get_x() + bar3.get_width()/2., h3 + 0.3,
             f'{h3:.1f}%', ha='center', va='bottom',
             fontsize=10, fontweight='bold')
    ax2.text(bar4.get_x() + bar4.get_width()/2., h4 + 0.3,
             f'{h4:.1f}%', ha='center', va='bottom',
             fontsize=10, fontweight='bold')

# Overall title
fig.suptitle(f'Biased vs Unbiased Prompt Comparison (Biased: Q3+Q4 N={biased_data[patterns[0]]["sample_size"]}, Unbiased: Full 2024 N={n_unbiased})',
...
# ============================================================================
# VERSION 3: Minimal Version (Publication Ready)
# ============================================================================
...
# Plot bars
bars1 = ax.bar(x - width/2, biased_detection, width, label='Biased',
               color='#4A90E2', alpha=0.95, edgecolor='black', linewidth=1)
bars2 = ax.bar(x + width/2, unbiased_detection, width, label='Unbiased',
               color='#E67E22', alpha=0.95, edgecolor='black', linewidth=1)
...
ax.set_title('Prompt Bias Impact on Detection Rates',
