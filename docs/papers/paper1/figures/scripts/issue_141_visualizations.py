...
def create_calendar_heatmap(df, output_path):
    ...
    for month in range(1, 13):
        ...
        for day in range(1, num_days + 1):
            ...
            # Draw square
            square = mpatches.Rectangle((weekday, 5-week), 0.9, 0.9,
            ...
            # Add day number
            if len(day_data) > 0:
                ax.text(weekday + 0.45, 5-week + 0.45, str(day),
    ...
    # Add legend
    fig.suptitle('Issue #141: Detection Status Calendar 2024',
    ...
    fig.legend(handles=[detection_patch, non_detection_patch],
...
def create_concentration_distribution(df, output_path):
    ...
    ax.hist(detected, bins=bins, alpha=0.6, color='#2ecc71',
           label=f'Detection Days (n={len(detected)}, μ={detected.mean():.4f})',
           edgecolor='black', linewidth=0.5)
    ax.hist(not_detected, bins=bins, alpha=0.6, color='#e74c3c',
           label=f'Non-Detection Days (n={len(not_detected)}, μ={not_detected.mean():.4f})',
           edgecolor='black', linewidth=0.5)

    # Add vertical lines for means
    ax.axvline(detected.mean(), color='#27ae60', linestyle='--', linewidth=2,
              label=f'Detection Mean: {detected.mean():.4f}')
    ax.axvline(not_detected.mean(), color='#c0392b', linestyle='--', linewidth=2,
...
def create_multifactor_scatter(df, output_path):
    ...
    ax1.scatter(detected['gex_concentration'], abs(detected['net_gex'])/1e9,
               alpha=0.6, s=50, color='#2ecc71', label='Detection', edgecolor='black', linewidth=0.5)
    ax1.scatter(not_detected['gex_concentration'], abs(not_detected['net_gex'])/1e9,
               alpha=0.6, s=50, color='#e74c3c', label='Non-Detection', edgecolor='black', linewidth=0.5)
    ...
    ax2.scatter(detected['gex_concentration'], detected['n_concentrated_strikes'],
               alpha=0.6, s=50, color='#2ecc71', label='Detection', edgecolor='black', linewidth=0.5)
    ax2.scatter(not_detected['gex_concentration'], not_detected['n_concentrated_strikes'],
               alpha=0.6, s=50, color='#e74c3c', label='Non-Detection', edgecolor='black', linewidth=0.5)
    ...
    ax3.scatter(abs(detected['net_gex'])/1e9, detected['put_call_ratio'],
               alpha=0.6, s=50, color='#2ecc71', label='Detection', edgecolor='black', linewidth=0.5)
    ax3.scatter(abs(not_detected['net_gex'])/1e9, not_detected['put_call_ratio'],
               alpha=0.6, s=50, color='#e74c3c', label='Non-Detection', edgecolor='black', linewidth=0.5)
    ...
    ax4.scatter(det_vol['gex_concentration'], det_vol['realized_vol_t1'],
               alpha=0.6, s=50, color='#2ecc71', label='Detection', edgecolor='black', linewidth=0.5)
    ax4.scatter(non_det_vol['gex_concentration'], non_det_vol['realized_vol_t1'],
               alpha=0.6, s=50, color='#e74c3c', label='Non-Detection', edgecolor='black', linewidth=0.5)
