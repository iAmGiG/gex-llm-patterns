...
def create_figure(confidence_data):
    ...
    for i, pattern in enumerate(pattern_order):
        ...
        ax.bar(x_pos, counts, width=bar_width,
...
def create_kde_figure(confidence_data):
    ...
    colors = {
        'gamma_positioning': '#2E86AB',
        'stock_pinning': '#A23B72',
        '0dte_hedging': '#F18F01'
    }
    ...
    for pattern, confidences in confidence_data.items():
        ...
        ax.plot(x, density, linewidth=2.5,
                label=f"{labels[pattern]} (N={len(confidences)})",
    ...
    ax.axvline(x=60, color='red', linestyle='--', linewidth=2,
               label='Mechanical Threshold (60%)', zorder=10)

    ax.set_xlabel('Confidence Score (%)', fontweight='bold')
    ax.set_ylabel('Probability Density', fontweight='bold')
    ax.set_title('Probability Density of Detection Confidence Scores',
...
def main():
    ...
    for pattern in patterns:
        ...
        if confidences:
            print(f"  {pattern}: {len(confidences)} detections, "
