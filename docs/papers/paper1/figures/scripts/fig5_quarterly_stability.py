...
def create_figure(quarterly_data, unbiased_data):
    ...
    line2 = ax2.plot(quarters, net_alphas, 's-', color=color_alpha,
                     linewidth=2.5, markersize=8, label='Net Alpha', zorder=4)
    ...
    legend_elements = [
        Line2D([0], [0], color=color_detection, linewidth=2.5, marker='o', markersize=8,
               label='Detection Rate (Biased)'),
        Line2D([0], [0], color=color_alpha, linewidth=2.5, marker='s', markersize=8,
               label='Net Alpha (bps)')
    ]
...
def main():
    ...
    for quarter, metrics in quarterly_data.items():
        print(f"  {quarter}: {metrics['detection_rate']:.1f}% detection, "
              f"{metrics['net_alpha']:+.1f} bps alpha")

    if unbiased_data:
        print(f"  Unbiased: {unbiased_data['detection_rate']:.1f}% detection, "
