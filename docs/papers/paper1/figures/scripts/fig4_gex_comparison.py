...
bars = ax.bar(strikes, gex_values, width=4.5, color=colors, alpha=0.85,
...
ax.axvline(x=spot_price, color='#3498DB', linestyle='--', linewidth=4,
           alpha=0.9, zorder=10)
...
ax.axvspan(atm_left, atm_right, alpha=0.2, color='yellow', zorder=1,
           label='ATM Region')

# ATM annotation (after dashed line to render on top)
ax.text((atm_left + atm_right) / 2, ax.get_ylim()[0] * 0.85,
...
ax.set_title(f'GEX Profile Example: Negative Gamma Regime\n(Representative pattern from {date})',
...
# LEFT: Negative GEX (from real data)
colors_neg = ['#E74C3C' if g < 0 else '#27AE60' for g in gex_values]
...
ax1.set_ylabel('Gamma Exposure ($ Billions)', fontsize=12, fontweight='bold')
ax1.set_title(f'(a) Negative Gamma Regime\nDealers SHORT gamma → Pro-cyclical hedging\n(From validation: {date})',
              fontsize=11, fontweight='bold', pad=10)
ax1.grid(axis='y', alpha=0.3, linestyle=':', linewidth=1)
ax1.set_xlim(strikes[0] - 3, strikes[-1] + 3)

# Net GEX label (center-right anchor to avoid blocking bars)
ax1.text(0.98, 0.5, f'Net GEX:\n${actual_net_gex:.1f}B',
...
net_gex_pos = np.sum(gex_values_pos)
colors_pos = ['#27AE60' if g > 0 else '#E74C3C' for g in gex_values_pos]
...
ax2.set_ylabel('Gamma Exposure ($ Billions)', fontsize=12, fontweight='bold')
ax2.set_title('(b) Positive Gamma Regime\nDealers LONG gamma → Counter-cyclical hedging\n(Hypothetical inverse for comparison)',
              fontsize=11, fontweight='bold', pad=10)
ax2.grid(axis='y', alpha=0.3, linestyle=':', linewidth=1)
ax2.set_xlim(strikes[0] - 3, strikes[-1] + 3)

# Net GEX label (center-right anchor to match left panel and avoid blocking bars)
ax2.text(0.98, 0.5, f'Net GEX:\n${net_gex_pos:.1f}B',
...
print("  • Strike distribution calibrated to match real net GEX")
print("  • Clear labeling as 'representative example' from validation")
print("  • Realistic magnitude and distribution based on actual market data")
