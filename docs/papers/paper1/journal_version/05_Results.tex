...
\label{fig:gex_concentration}
\end{figure}

Table~\ref{tab:nondetection} summarizes the statistical comparison across all tested factors. Three characteristics emerge as significant discriminators:
...
\begin{tabular}{lccc}
\toprule
Factor & Detection & Non-Detection & p-value \\
 & Mean (SD) & Mean (SD) & \\
\midrule
\textbf{GEX Concentration} & \textbf{-0.853} & \textbf{-0.866} & \textbf{<0.0001***} \\
 & (0.020) & (0.017) & \\
GEX Magnitude (\$B) & 31.8 & 30.3 & 0.007** \\
 & (4.1) & (4.2) & \\
Concentrated Strikes & 2.27 & 1.84 & 0.027* \\
 & (1.27) & (1.63) & \\
\midrule
Realized Volatility (\%) & 0.634 & 0.577 & 0.464 \\
Rolling Volatility (\%) & 0.776 & 0.662 & 0.076 \\
Put-Call Ratio & 1.065 & 1.009 & 0.297 \\
Options Count & 9,080 & 9,191 & 0.319 \\
\bottomrule
\multicolumn{4}{l}{\footnotesize * p < 0.05, ** p < 0.01, *** p < 0.001}
\end{tabular}
\end{table}
...
Figure~\ref{fig:detection_calendar} visualizes the temporal distribution of non-detection days throughout 2024, confirming non-random clustering during periods of dispersed gamma positioning.

\begin{figure}[!htbp]
\centering
\includegraphics[width=\columnwidth]{../figures/issue_141_detection_calendar.png}
...
\label{fig:detection_calendar}
\end{figure}

\subsubsection{Materialization Specificity (P-Hacking Defense)}

To address potential concerns that pattern detection reflects p-hacking (detecting patterns that universally predict common outcomes), we analyzed materialization rates for two outcome criteria across 519 detection days versus 100 randomly sampled non-detection days (Table~\ref{tab:materialization}).
...
\toprule
Criterion & Detection & Baseline & Lift & $\chi^2$ & p \\
\midrule
Volatility Amplification (C1) & 41.6\% & 45.0\% & 0.92$\times$ & 0.27 & 0.61 \\
Range Expansion (C4) & 21.6\% & 32.0\% & 0.67$\times$ & 4.53 & 0.03$^*$ \\
\bottomrule
\end{tabular}
\end{table}
