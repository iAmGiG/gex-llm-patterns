...
Interestingly, LLM detection confidence (mean 79.5\%, std 4.0\%) achieves AUC = 0.488—below random chance for predicting T+1 outcomes. This divergence reveals an important distinction: the LLM's value lies in \emph{pattern identification and causal interpretation}, not probability calibration. The statistical model quantifies T+1 outcome likelihood through GEX features, while the LLM identifies the structural mechanism (dealer hedging constraints) without accurately predicting \emph{when} those constraints materialize. This complementarity—LLM for mechanism identification, statistical models for quantification—suggests hybrid approaches may optimize both interpretability and prediction accuracy.
