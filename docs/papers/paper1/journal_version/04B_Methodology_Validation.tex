...
\textbf{Sample Size Consideration.} While the raw chain validation uses 13 dates (constrained by raw options chain data availability and computational cost of strike-level processing), the 30.8 percentage point effect size substantially exceeds the minimum detectable effect for this sample ($\sim$15pp at 80\% power), providing statistically robust evidence despite the smaller $n$. Future work with expanded raw chain coverage would further strengthen this finding.
