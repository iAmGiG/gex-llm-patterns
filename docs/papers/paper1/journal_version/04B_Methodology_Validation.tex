...
Figure~\ref{fig:raw_chain_comparison} visualizes this comparison, showing the raw chain method achieves 92.3\% detection (12/13 cases), \textbf{outperforming} the GEX-assisted baseline by 30.8 percentage points. This result directly refutes the pattern-matching critique: when we remove the ``\$-32B GEX'' label entirely, the LLM's detection \textit{improves}.

\begin{figure}[htbp]
\centering
\includegraphics[width=\columnwidth]{../figures/fig3_raw_chain.pdf}
...
\label{fig:raw_chain_comparison}
\end{figure}
