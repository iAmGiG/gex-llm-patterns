...
Table~\ref{tab:raw-chain-comparison} presents the detection rates comparing raw chain inputs against our standard GEX-assisted baseline on the same 13 test dates.

\begin{table}[!htbp]
\centering
\caption{Detection Performance: Raw Chain vs. GEX-Assisted Baseline}
\label{tab:raw-chain-comparison}
\begin{tabular}{lcc}
\toprule
...
The raw chain method achieves 92.3\% detection (12/13 cases), \textbf{outperforming} the GEX-assisted baseline by 30.8 percentage points. This result directly refutes the pattern-matching critique: when we remove the ``\$-32B GEX'' label entirely, the LLM's detection \textit{improves}.
