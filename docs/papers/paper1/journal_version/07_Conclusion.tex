...
\textbf{Pillar 2: Inverse P-Hacking.} Detection days exhibit 33\% \textit{lower} intraday range expansion than materialization days (21.6\% vs. 32.0\%, p = 0.03), demonstrating the LLM detects \textit{volatility suppression} rather than chasing high-volatility outcomes. Combined with 91.2\% materialization accuracy, this proves pattern-specific detection rather than a wide net capturing random volatility. The divergence between stable detection (68-74\% across quarters) and declining economic profitability (Sharpe 1.8 $\rightarrow$ 0.1) further confirms mechanism-based reasoning independent of alpha-generating capacity.
...
\begin{enumerate}
    \item \textbf{Sensitivity vs. Guessing} (Issue \#141): Non-detection days have 3.72$\times$ weaker GEX signal concentration (p < 0.0001, Cohen's d = 0.68). The LLM requires concentrated structural constraints, refuting base rate guessing.

    \item \textbf{Inverse P-Hacking} (Issue \#144): Detection days show 33\% lower range expansion than materialization days (p = 0.03). The LLM detects suppression, not volatility chasing.

    \item \textbf{Not Profit-Chasing} (Issue \#146): Confidence increased (79.5 $\rightarrow$ 80.7) Q1-Q4 2024 while net alpha collapsed (Sharpe 1.8 $\rightarrow$ 0.1). The LLM identifies mechanisms independent of profitability.

    \item \textbf{EOD Validity} (Issue \#145): Statistical baseline achieves AUC = 0.681 predicting T+1 outcomes from EOD features. GEX normalized by OI (p = 0.0075) is the only significant predictor, validating EOD structural snapshots.

    \item \textbf{Structural Analyst} (Issue \#143): Raw chain (92.3\%) outperforms GEX-assisted (61.5\%) baseline by 30.8pp. The LLM reconstructs dealer constraints from first principles, not metric pattern-matching.
\end{enumerate}
...
The inverse p-hacking finding challenges conventional validation assumptions. Rather than seeking high-volatility outcomes (traditional p-hacking), the LLM detects \textit{suppression}—days when dealer hedging mechanically dampens realized volatility below the predicted range. This asymmetry (detecting constraints rather than their absence) provides strong evidence for mechanism-specific reasoning over correlation fishing.
