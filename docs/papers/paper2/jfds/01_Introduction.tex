...
The explosive growth of zero-days-to-expiration (0DTE) options since 2021 has fundamentally altered dealer hedging dynamics in equity index markets, yet detecting the resulting structural shifts remains challenging for traditional quantitative approaches. Simple threshold rules miss nuanced regime transitions, while classical econometric methods struggle with the high-dimensional, non-stationary nature of gamma exposure sequences. We introduce a \textit{structural reasoning framework} using large language models as a systemic heuristic for microstructure analysis. Building on our companion paper's single-day constraint detection~\citep{regan2025obfuscation}, we extend to 30-day windows that capture \textit{persistent regimes}—sustained periods where dealer positioning exhibits structural coherence lasting weeks or months.

This temporal extension matters because markets exhibit different orders of complexity. First, single-day detection validates constraint recognition at a point in time but cannot distinguish emergent persistence from transitional fragmentation. Second, the spontaneous proliferation of zero-days-to-expiration (0DTE) options since 2021~\citep{cboe2024zero,dim2025zero} fundamentally altered the incentive structures facing dealers, potentially inducing more persistent coordination patterns. Third, regime boundaries offer actionable insights for risk management and sector rotation strategies, whereas daily snapshots provide limited forward guidance about structural evolution.

We address this gap by extending obfuscation-based validation from single-day patterns to \textbf{30-day regime windows}. Unlike prior work detecting daily hedging flows (present in all markets since Black-Scholes~\citep{black1973pricing}), we test whether LLMs can identify \textit{persistent structural regimes}—sustained periods where dealer positioning exhibits consistent directional bias. This selectivity is critical: detecting universal patterns proves only pattern matching, while discriminating regimes from transitional periods validates structural reasoning.
...
This work advances structural reasoning for market microstructure analysis through five contributions:
...
\item \textbf{High-Fidelity Discrimination}: Framework achieves 5.7x separation between persistent regimes (2024: 81.2\%) and fragmented markets (2020: 12.1\%), demonstrating discriminative power that simple threshold rules cannot match
...
\item \textbf{Ablation and Robustness}: Comprehensive validation protocols against data leakage through synthetic windows, establishing $<$10\% false positive rate on transitional/low-magnitude scenarios
\end{enumerate}
...
\item \textbf{Discriminative power demonstrated}: The 69.1 percentage point gap between 2024 (81.2\%) and 2020 (12.1\%) demonstrates the framework identifies structural persistence that arithmetic thresholds miss. The 2020 baseline correctly rejected 87.9\% of windows as transitional.
