...
\textbf{Framework-dependent reasoning}: Preliminary ablation testing (n=84 balanced sample) suggests the WHO→WHOM→WHAT narrative framework may not be strictly necessary for detection accuracy—both narrative and data-only prompts achieved comparable classification performance. However, the framework provides interpretability benefits: auditable reasoning chains, mechanism-grounded explanations, and enhanced human oversight of LLM decisions. Full validation with larger samples and cross-period testing would clarify whether interpretability scaffolding becomes detection-critical in edge cases.
