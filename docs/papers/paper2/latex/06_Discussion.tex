...
\textbf{(B)} Phase change schematic at 4:00 PM expiration. Before expiration, dealers hold both gamma exposure (\$15B from 0DTE options) and corresponding hedge positions ($\sim$\$15B stock). At expiration, gamma vanishes instantly as options expire worthless or are exercised, but the hedge position remains. This residual stock position must be unwound during overnight/T+1 sessions, creating the persistent positioning captured in EOD open interest measurements.
}
...
\noindent\textbf{Supplementary evidence: The ``Great Divergence''}: The scar tissue mechanism implies a testable consequence—if dealers carry overnight positioning, returns should shift from intraday to overnight sessions. Figure~\ref{fig:divergence} decomposes SPY total returns into overnight (Close$_T$ → Open$_{T+1}$) and intraday (Open$_{T+1}$ → Close$_{T+1}$) components across 2020-2025, revealing a striking temporal pattern: Panel~A's cumulative ``jaw'' chart shows overnight and intraday returns tracking together through 2021, then diverging sharply as 0DTE adoption accelerated; Panel~B's annual breakdown quantifies the shift—pre-0DTE (2020-2021), overnight and intraday returns were roughly balanced, while post-adoption (2024), overnight returns dominate (+21.2\%) and intraday returns compressed to near-zero (+0.6\%). This pattern is \textit{consistent with} the scar tissue hypothesis—dealers extracting risk premium from overnight carry—though we emphasize correlation rather than causation. Alternative explanations (interest rate environment changes, market maker behavior shifts) cannot be excluded.

\begin{figure*}[!htbp]
\centering
\includegraphics[width=0.95\textwidth]{../figures/output/fig12_overnight_intraday_divergence.png}
