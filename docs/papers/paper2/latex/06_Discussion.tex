...
\noindent\textbf{Explaining the 2023 detection dip}: A critical pattern in Phase 5 results requires explanation: detection \textit{declined} from 32.4\% (2022) to 20.2\% (2023) before exploding to 100\% (2024). If 0DTE adoption was monotonically increasing, why did regime persistence temporarily regress? We interpret 2023 as a ``threshold resistance'' period---0DTE volumes had grown substantially (reaching 43\% share) but had not yet crossed the critical mass required to overwhelm dealer inventory limits \textit{consistently}. During 2023, the market repeatedly \textit{attempted} to form persistent regimes but frequently failed the magnitude or stability criteria (hence the ``Inconsistent'' classification in Table VII). The suppressed volatility environment of 2023 (the ``short vol'' trade) may have allowed dealers to absorb 0DTE hedging pressure without sustained positioning buildup. Only when 0DTE volumes reached saturation levels ($\approx$48\%) in late 2023 did dealer inventory constraints become binding across \textit{all} market conditions, producing the structural transition to 100\% detection in 2024. This non-monotonic pattern actually \textit{strengthens} the 0DTE hypothesis: it demonstrates a tipping point dynamic rather than gradual linear adoption, consistent with inventory constraint models where dealers can absorb pressure until a threshold is exceeded.
...
\noindent\textbf{Formula Agreement Test}: To determine whether regime detection depends on absolute magnitude or structural patterns, we executed the \textit{Formula Agreement Test} on Q1 2024 (61 windows). Using sign-preserving normalization (dividing by global maximum magnitude to scale values to $[-1, +1]$ while maintaining directional structure), we compared regime classifications between absolute-scaled and normalized GEX formulations. The result: \textbf{100\% agreement rate}---every window received identical regime classification regardless of scaling. This demonstrates that our detection methodology is \textit{calculation-independent}: the LLM identifies regimes based on structural patterns (persistence, sign consistency, flip frequency) rather than absolute dollar values. This finding has practical implications for deploying LLM-based detection across different financial data representations where absolute magnitudes may vary.
