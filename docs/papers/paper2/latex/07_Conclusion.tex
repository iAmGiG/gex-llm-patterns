...
We extended obfuscation-based LLM validation from single-day dealer gamma constraints to persistent 30-day market regimes, addressing a critical gap in temporal structural reasoning. Four-phase validation across 446 windows spanning two critical years---2020 (pre-0DTE) and 2024 (post-0DTE)---demonstrates that LLMs can identify sustained dealer positioning through three structural criteria: persistence ($\geq$70\% same-sign dominance), magnitude ($\geq$\$5B average), and stability ($\leq$5 sign flips).
...
\textbf{Large 2020→2024 market structure difference identified}: Phase 4 comparison (446 windows across 2020 and 2024) revealed a 69.1 percentage point detection difference between pre-0DTE (12.1\%) and post-0DTE (81.2\%) eras. This large effect size ($\phi$ = 0.672) temporally coincides with 0DTE proliferation (2020 $<$5\% volume → 2024 43\% volume), supporting the hypothesis that 0DTE options create persistent dealer gamma regimes.
...
\item \textbf{Cost-effective comprehensive testing}: OpenAI Batch API enabled validation at approximately \$0.49 reproduction cost (50\% savings), making rigorous cross-era testing economically feasible for academic research
\end{enumerate}
...
\textbf{Temporal reasoning capabilities}: LLMs can identify persistent structural constraints across 30-day windows when given appropriate prompts and criteria. The 98\% mechanical accuracy across 446 validation windows suggests robust computational reasoning about dealer hedging mechanics.
...
\textbf{Multi-year validation}: Our validation covers only 2020 and 2024. Extending to intermediate years (2021-2023) would identify the precise timing of structural transition. Longer-term historical analysis (2005-2020) could reveal whether similar regime transitions occurred during earlier options market innovations such as 2008-2010 (financial crisis), 2016-2018 (VIX ETF proliferation), and 2005-2007 (pre-crisis baseline).
...
\textbf{Framework ablation study}: Testing whether LLMs need the WHO→WHOM→WHAT causal framework or can detect constraints from data alone will clarify the role of prompt structure in enabling structural reasoning.
