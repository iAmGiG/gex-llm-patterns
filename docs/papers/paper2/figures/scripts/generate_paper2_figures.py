...
def create_multiyear_detection_chart(output_path):
    ...
    ax.annotate('Sharp Structural\nTransition',
                xy=(2020.5, 50), xytext=(2020.5, 70),
    ...
    ax.set_title('Persistent Regime Detection Across 6 Years (2020-2025)',
    ...
    ax.text(0.98, 0.98, '2020→2021: 87.9 pp increase\np < 10⁻⁸⁶, φ = 0.909',
...
def create_2020_vs_2024_comparison(output_path):
    ...
    bars1 = ax.bar(x - width/2, data_2020_viz, width, label='2020 (Pre-0DTE)',
                   color='#e74c3c', alpha=0.8, edgecolor='black', linewidth=1.5)
    bars2 = ax.bar(x + width/2, data_2024_viz, width, label='2024 (Post-0DTE)',
    ...
    for i, (b1, b2) in enumerate(zip(bars1, bars2)):
        ...
        if i == 3:  # Magnitude - show actual $B values
            ax.text(b1.get_x() + b1.get_width()/2, b1.get_height() + 2,
                   f'${val1:.1f}B', ha='center', va='bottom', fontsize=9)
            ax.text(b2.get_x() + b2.get_width()/2, b2.get_height() + 2,
                   f'${val2:.1f}B', ha='center', va='bottom', fontsize=9)
        else:
            ax.text(b1.get_x() + b1.get_width()/2, b1.get_height() + 2,
    ...
    ax.set_title('Market Structure Comparison: 2020 vs 2024',
                 fontsize=14, weight='bold', pad=20)
    ax.set_xticks(x)
    ...
    ax.text(0.98, 0.02, '69.1 pp detection increase\n4.9x magnitude increase',
            transform=ax.transAxes, fontsize=9,
...
def create_negative_controls_chart(output_path):
    ...
    bars1 = ax.bar(x - width/2, fp_2024, width, label='2024 Q1 FP Rate',
                   color='#3498db', alpha=0.8, edgecolor='black', linewidth=1.5)
    bars2 = ax.bar(x + width/2, fp_2020, width, label='2020 FP Rate',
                   color='#9b59b6', alpha=0.8, edgecolor='black', linewidth=1.5)

    # Add threshold lines
    threshold_values = [20, 10, 10]
    for i, thresh in enumerate(threshold_values):
        ax.hlines(thresh, i - 0.5, i + 0.5, colors='red', linestyles='dashed',
                  linewidth=2, label='Threshold' if i == 0 else '')
    ...
    for i, test in enumerate(tests):
        ...
        ax.text(i - width/2, -3, status_2024, ha='center', va='top',
                fontsize=8, weight='bold', color=color_2024)
        ax.text(i + width/2, -3, status_2020, ha='center', va='top',
                fontsize=8, weight='bold', color=color_2020)

    ax.set_ylabel('False Positive Rate (%)', fontsize=12, weight='bold')
    ax.set_title('Phase 2 Negative Controls Validation',
                 fontsize=14, weight='bold', pad=20)
    ax.set_xticks(x)
    ...
    # Add interpretation
    ax.text(0.02, 0.98, 'Framework selectivity validated:\n0% FP on transitional/low-mag tests',
            transform=ax.transAxes, fontsize=9,
...
def create_gex_magnitude_evolution(output_path):
    ...
    # Plot main line
    ax.plot(years, gex_magnitude, marker='o', markersize=10, linewidth=3,
            color='#2c3e50', label='Avg GEX Magnitude')
    ...
    ax.annotate('', xy=(2021, 27.2), xytext=(2020, 17.3),
                arrowprops=dict(arrowstyle='<->', lw=2, color='#e74c3c'))
    ax.text(2020.5, 22, '+58%', ha='center', fontsize=10, weight='bold',
            bbox=dict(boxstyle='round', facecolor='yellow', alpha=0.5))

    ax.set_xlabel('Year', fontsize=12, weight='bold')
    ax.set_ylabel('Average GEX Magnitude ($B)', fontsize=12, weight='bold')
    ax.set_title('Gamma Exposure Evolution: Pre vs Post-0DTE Era',
                 fontsize=14, weight='bold', pad=20)
    ax.set_xticks(years)
