...
def create_simple_figure_6():
    ...
    draw.rectangle([box_x, box_y, box_x + box_w, box_y + box_h],
                  outline=COLORS['black'], fill=COLORS['light_green'], width=2)
...
def create_simple_figure_7():
    ...
    for pos, (title, metrics, detected, reason) in zip(positions, examples):
        ...
        draw.rectangle([x - box_size//2, y - 100, x + box_size//2, y + 200],
                       outline=COLORS['black'], fill=color, width=2)
...
def create_simple_figure_8():
    ...
    y_positions = [120, 220, 320, 420, 520]
    widths = [400, 450, 400, 400, 500]
    ...
    for i, ((phase, metrics), y, width) in enumerate(zip(phases, y_positions, widths)):
        ...
        draw.rectangle([x - width//2, y, x + width//2, y + h],
                      outline=COLORS['blue'], fill=(173, 216, 230), width=2)
