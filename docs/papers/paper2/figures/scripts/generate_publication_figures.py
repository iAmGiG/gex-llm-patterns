...
def draw_arrow(draw, start, end, color, width=3, head_size=12):
    ...
    p1 = (x2 - head_size * math.cos(angle - head_angle),
          y2 - head_size * math.sin(angle - head_angle))
    p2 = (x2 - head_size * math.cos(angle + head_angle),
          y2 - head_size * math.sin(angle + head_angle))
...
def create_figure_5():
    ...
    # Title
    draw.text((width // 2, 35), "Temporal Obfuscation Process",
              fill=COLORS['black'], font=fonts['subtitle'], anchor="mm")

    # === Left Box: Original Data ===
    box_w, box_h = 380, 280
    left_x, left_y = 80, 100

    draw_rounded_rect(draw, [left_x, left_y, left_x + box_w, left_y + box_h],
                      radius=15, fill=COLORS['bg_blue'], outline=COLORS['detected'], width=3)

    draw.text((left_x + box_w // 2, left_y + 35), "Original Data",
              fill=COLORS['detected'], font=fonts['label'], anchor="mm")
    ...
    draw.line([(arrow_start_x, arrow_y), (arrow_end_x - 15, arrow_y)],
              fill=COLORS['dark_gray'], width=4)
    draw_arrow(draw, (arrow_end_x - 30, arrow_y), (arrow_end_x, arrow_y),
               COLORS['dark_gray'], width=4, head_size=15)

    # Arrow label
    draw.text((mid_x, arrow_y - 30), "Obfuscate", fill=COLORS['dark_gray'],
              font=fonts['label'], anchor="mm")

    # === Right Box: Obfuscated Data ===
    right_x = width - 80 - box_w

    draw_rounded_rect(draw, [right_x, left_y, right_x + box_w, left_y + box_h],
                      radius=15, fill=COLORS['bg_green'], outline=COLORS['success'], width=3)

    draw.text((right_x + box_w // 2, left_y + 35), "Obfuscated Data",
              fill=COLORS['success'], font=fonts['label'], anchor="mm")
...
def create_figure_6():
    ...
    # Title
    draw.text((width // 2, 35), "30-Day Persistent Negative Regime Example",
              fill=COLORS['black'], font=fonts['subtitle'], anchor="mm")
    ...
    for gex in range(-20, 10, 5):
        ...
        # Y-axis label
        draw.text((chart_left - 15, y), f"${gex}B", fill=COLORS['dark_gray'],
                  font=fonts['axis'], anchor="rm")

    # Y-axis title (rotated text simulation - just horizontal for now)
    draw.text((50, (chart_top + chart_bottom) // 2), "Net GEX",
              fill=COLORS['black'], font=fonts['label'], anchor="mm")

    # Simulated 30-day GEX data (based on Phase 4A 2024 patterns)
    # Persistent negative with 2 sign flips (days 8, 22)
    gex_data = [
        -14.2, -12.8, -15.1, -13.5, -16.2, -11.9, -14.7, +3.2,  # Days 1-8
        -13.1, -15.8, -12.4, -14.9, -16.5, -11.2, -13.8, -15.3,  # Days 9-16
        -12.7, -14.1, -16.8, -13.2, -15.4, +2.8, -14.5, -12.9,  # Days 17-24
        -15.7, -13.4, -16.1, -14.3, -12.6, -15.9                 # Days 25-30
    ]
    ...
    for i, gex in enumerate(gex_data):
        ...
        if (i + 1) % 5 == 0 or i == 0:
            draw.text((x + bar_width // 2, chart_bottom + 20), str(i + 1),
                      fill=COLORS['dark_gray'], font=fonts['tiny'], anchor="mm")

    # X-axis label
    draw.text(((chart_left + chart_right) // 2, chart_bottom + 50), "Trading Day",
              fill=COLORS['black'], font=fonts['label'], anchor="mm")
    ...
    draw_rounded_rect(draw, [box_x, box_y, box_x + box_w, box_y + box_h],
                      radius=15, fill=COLORS['bg_gray'], outline=COLORS['dark_gray'], width=2)

    draw.text((box_x + box_w // 2, box_y + 30), "Detection Criteria",
              fill=COLORS['black'], font=fonts['label'], anchor="mm")
    ...
    for label, value, check, passed in criteria:
        if label:
            ...
            if passed is not None:
                ...
                draw.text((box_x + box_w - 40, crit_y), "PASS" if passed else "FAIL",
                          fill=check_color, font=fonts['small'])
        else:
    ...
    draw_rounded_rect(draw, [box_x + 40, badge_y, box_x + box_w - 40, badge_y + 45],
                      radius=8, fill=COLORS['success'], outline=None)
    draw.text((box_x + box_w // 2, badge_y + 22), "PERSISTENT REGIME DETECTED",
              fill=COLORS['white'], font=fonts['small'], anchor="mm")
...
def create_figure_7():
    ...
    # Title
    draw.text((width // 2, 40), "Framework Selectivity: Regime Classification",
              fill=COLORS['black'], font=fonts['subtitle'], anchor="mm")
    ...
    for (x, y), ex in zip(positions, examples):
        ...
        draw_rounded_rect(draw, [x, y, x + cell_w, y + cell_h],
                          radius=15, fill=ex['color'], outline=ex['border'], width=3)

        # Title
        draw.text((x + cell_w // 2, y + 30), ex['title'],
                  fill=COLORS['black'], font=fonts['label'], anchor="mm")

        # Year badge
        badge_color = COLORS['detected'] if ex['year'] == '2024' else COLORS['transitional']
        draw_rounded_rect(draw, [x + cell_w - 80, y + 10, x + cell_w - 15, y + 40],
                          radius=5, fill=badge_color, outline=None)
        draw.text((x + cell_w - 47, y + 25), ex['year'],
                  fill=COLORS['white'], font=fonts['tiny'], anchor="mm")
        ...
        for label, value, passed in metrics:
            ...
            draw.text((x + cell_w - 70, metrics_y), indicator_text,
                      fill=indicator_color, font=fonts['small'])

            metrics_y += 45

        # Horizontal divider
        draw.line([(x + 25, metrics_y + 10), (x + cell_w - 25, metrics_y + 10)],
                  fill=COLORS['light_gray'], width=2)
        ...
        # Reason
        draw.text((x + 30, result_y + 40), ex['reason'],
                  fill=COLORS['medium_gray'], font=fonts['small'])
    ...
    for color, text in legend_items:
        ...
        draw.text((legend_x + 35, legend_y + 12), text, fill=COLORS['dark_gray'],
                  font=fonts['small'], anchor="lm")
        legend_x += 400
...
def create_figure_8():
    ...
    # Title
    draw.text((width // 2, 35), "Multi-Phase Validation Pipeline",
              fill=COLORS['black'], font=fonts['subtitle'], anchor="mm")
    ...
    for i, phase in enumerate(phases):
        ...
        # Draw chevron
        draw_chevron(x, y, chevron_w, chevron_h, chevron_point,
                     fill=phase['color'], outline=COLORS['dark_gray'])

        # Phase name (white text on colored background)
        text_x = x + chevron_w // 2
        draw.text((text_x, y + 30), phase['name'],
                  fill=COLORS['white'], font=fonts['label'], anchor="mm")
        draw.text((text_x, y + 60), phase['subtitle'],
                  fill=COLORS['white'], font=fonts['small'], anchor="mm")

        # Metrics below chevron
        metric_y = y + chevron_h + 30
        draw.text((text_x, metric_y), phase['desc'],
                  fill=COLORS['dark_gray'], font=fonts['small'], anchor="mm")
        draw.text((text_x, metric_y + 30), f"n = {phase['windows']}",
                  fill=COLORS['dark_gray'], font=fonts['axis'], anchor="mm")
        draw.text((text_x, metric_y + 60), phase['detection'],
                  fill=COLORS['black'], font=fonts['label'], anchor="mm")

    # Summary box at bottom
    summary_y = 480
    draw_rounded_rect(draw, [80, summary_y, width - 80, summary_y + 80],
                      radius=10, fill=COLORS['bg_gray'], outline=COLORS['dark_gray'], width=2)

    summary_text = "Key Finding: Detection rate increased from 12.1% (2020) to 81.2% (2024) following 0DTE market structure shift"
    draw.text((width // 2, summary_y + 40), summary_text,
              fill=COLORS['black'], font=fonts['text'], anchor="mm")
...
def create_figure_9():
    ...
    # Title
    draw.text((width // 2, 35), "Detection Rate by Year: 0DTE Market Structure Transition",
              fill=COLORS['black'], font=fonts['subtitle'], anchor="mm")
    ...
    for pct in range(0, 101, 20):
        ...
        draw.text((chart_left - 15, y), f"{pct}%", fill=COLORS['dark_gray'],
                  font=fonts['axis'], anchor="rm")

    # Y-axis label
    draw.text((60, (chart_top + chart_bottom) // 2), "Detection Rate",
              fill=COLORS['black'], font=fonts['label'], anchor="mm")
    ...
    for i, year in enumerate(years):
        x = x_to_pixel(i)
        draw.text((x, chart_bottom + 25), year, fill=COLORS['dark_gray'],
                  font=fonts['text'], anchor="mm")
        draw.text((x, chart_bottom + 50), f"n={windows[i]}", fill=COLORS['medium_gray'],
    ...
    for i, (x, y) in enumerate(points):
        ...
        draw.ellipse([x - point_radius, y - point_radius,
                     ...
                     fill=COLORS['detected'], outline=COLORS['white'])
        # Rate label
        rate = detection_rates[i]
        label_y = y - 30 if rate > 50 else y + 30
        draw.text((x, label_y), f"{rate:.1f}%", fill=COLORS['black'],
    ...
    for y in range(chart_top, chart_bottom, dash_len * 2):
        draw.line([(transition_x, y), (transition_x, min(y + dash_len, chart_bottom))],
                  fill=COLORS['highlight'], width=3)
    ...
    draw_rounded_rect(draw, [ann_x, ann_y, ann_x + ann_w, ann_y + ann_h],
                      radius=8, fill=COLORS['white'], outline=COLORS['highlight'], width=2)

    draw.text((ann_x + ann_w // 2, ann_y + 25), "0DTE Launch",
              fill=COLORS['highlight'], font=fonts['label'], anchor="mm")
    draw.text((ann_x + ann_w // 2, ann_y + 55), "May 2022 (SPY)",
              fill=COLORS['dark_gray'], font=fonts['small'], anchor="mm")
    draw.text((ann_x + ann_w // 2, ann_y + 80), "Market structure shift",
              fill=COLORS['medium_gray'], font=fonts['tiny'], anchor="mm")

    # Arrow from annotation to transition line
    draw_arrow(draw, (ann_x, ann_y + 50), (transition_x + 5, ann_y + 50),
               COLORS['highlight'], width=2, head_size=10)

    # Summary at bottom
    summary_y = 620
    summary_text = "8.3x increase in detection rate from 2020 to 2021 (p < 0.0001, phi = 0.672)"
    draw.text((width // 2, summary_y), summary_text,
              fill=COLORS['dark_gray'], font=fonts['text'], anchor="mm")
...
def create_figure_10():
    ...
    # Title
    draw.text((width // 2, 35), "LLM Regime Detection System Architecture",
              fill=COLORS['black'], font=fonts['subtitle'], anchor="mm")
    ...
    components = [
        {
            ...
            'x': 180, 'y': 150, 'w': 280, 'h': 200,
            'color': COLORS['bg_blue'], 'border': COLORS['detected'],
        },
        {
            'name': "GEX\nCalculation",
            'items': ["Net gamma exposure", "OI vs Volume GEX", "Strike aggregation"],
            'x': 560, 'y': 150, 'w': 280, 'h': 200,
            'color': COLORS['bg_blue'], 'border': COLORS['detected'],
        },
        {
            'name': "Temporal\nObfuscation",
            'items': ["Date masking", "Symbol aliasing", "Relative strikes"],
            'x': 940, 'y': 150, 'w': 280, 'h': 200,
            'color': COLORS['bg_green'], 'border': COLORS['success'],
        },
        {
            'name': "30-Day\nWindows",
            'items': ["Rolling windows", "Regime detection", "1,418 windows"],
            'x': 1320, 'y': 150, 'w': 280, 'h': 200,
            'color': COLORS['bg_green'], 'border': COLORS['success'],
        },
        {
            'name': "LLM\nAnalysis",
            'items': ["OpenAI o4-mini", "Batch API", "Regime prediction"],
            'x': 1700, 'y': 150, 'w': 280, 'h': 200,
            'color': COLORS['bg_gray'], 'border': COLORS['transitional'],
        },
    ]
    ...
    for comp in components:
        ...
        draw_rounded_rect(draw, [x, y, x + w, y + h],
                          radius=15, fill=comp['color'], outline=comp['border'], width=3)

        # Component name
        name_lines = comp['name'].split('\n')
        name_y = y + 35
        for line in name_lines:
            draw.text((x + w // 2, name_y), line,
                      fill=COLORS['black'], font=fonts['label'], anchor="mm")
            name_y += 28
    ...
    for i in range(len(components) - 1):
        ...
        x2 = components[i + 1]['x']
        draw_arrow(draw, (x1 + 10, arrow_y), (x2 - 10, arrow_y),
                   COLORS['dark_gray'], width=3, head_size=12)

    # Output box at bottom
    out_x, out_y = width // 2 - 300, 420
    out_w, out_h = 600, 160

    draw_rounded_rect(draw, [out_x, out_y, out_x + out_w, out_y + out_h],
                      radius=15, fill=COLORS['bg_gray'], outline=COLORS['black'], width=2)

    draw.text((out_x + out_w // 2, out_y + 30), "Validation Results",
              fill=COLORS['black'], font=fonts['label'], anchor="mm")
    ...
    for result in results:
        draw.text((out_x + out_w // 2, result_y), result,
                  fill=COLORS['dark_gray'], font=fonts['text'], anchor="mm")
        result_y += 30

    # Arrow from LLM to output
    draw_arrow(draw, (components[-1]['x'] + components[-1]['w'] // 2,
                      components[-1]['y'] + components[-1]['h'] + 10),
    ...
    draw_rounded_rect(draw, [db_x, db_y, db_x + db_w, db_y + db_h],
                      radius=10, fill=COLORS['white'], outline=COLORS['detected'], width=2)

    draw.text((db_x + db_w // 2, db_y + 30), "SQLite DB",
              fill=COLORS['detected'], font=fonts['text'], anchor="mm")
    draw.text((db_x + db_w // 2, db_y + 60), "3.25 GB",
              fill=COLORS['dark_gray'], font=fonts['small'], anchor="mm")
    draw.text((db_x + db_w // 2, db_y + 85), "11.8M options",
              fill=COLORS['medium_gray'], font=fonts['tiny'], anchor="mm")
    ...
    draw_arrow(draw, (components[1]['x'] + components[1]['w'] // 2,
                      components[1]['y'] + components[1]['h'] + 10),
               (db_x + db_w // 2, db_y - 10),
               COLORS['detected'], width=2, head_size=10)
    ...
    for color, label in key_items:
        draw.rectangle([key_x, key_y, key_x + 30, key_y + 30],
                       fill=color, outline=COLORS['dark_gray'])
        draw.text((key_x + 45, key_y + 15), label,
                  fill=COLORS['dark_gray'], font=fonts['small'], anchor="lm")
        key_x += 350
