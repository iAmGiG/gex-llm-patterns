...
class PatternEngineIntegration:
    ...
    def __init__(self, historical_database_path: str = None):
        ...
        self.probability_mapper = PatternProbabilityMapper(
    ...
    def analyze_current_patterns(self,
        ...
        fed_summary = self.fed_analyzer.create_context_summary(
            analysis_timestamp)
        ...
        adjusted_patterns = self._apply_fed_adjustments(
            detected_patterns, pattern_adjustments)

        # 5. Calculate confidence scores (if historical data available)
        confidence_scores = self._calculate_pattern_confidences(
            adjusted_patterns, fed_summary
        )
    ...
    def run_historical_analysis(self,
        ...
        if start_date or end_date:
            historical_data = self._filter_date_range(
        ...
        statistical_results = self.statistical_validator.calculate_significance(
            pattern_outcomes)
    ...
    def generate_comprehensive_report(self,
        ...
        if output_path is None:
            ...
            output_path = Path('.cache/pattern_analysis') / \
                f"comprehensive_report_{timestamp}.txt"
        ...
        if current_analysis:
            ...
            if current_analysis['detected_patterns']:
                ...
                for pattern in current_analysis['detected_patterns']:
                    ...
                    report_lines.append(
                        f"  • {pattern['pattern'].upper()}: {conf}% confidence")
                    report_lines.append(
                        f"    └─ {pattern.get('details', 'No details')}")
                report_lines.append("")
        ...
        if historical_analysis:
            ...
            if historical_analysis.get('statistically_significant_patterns'):
                ...
                report_lines.extend([
                    ...
                    f"  Total: {len(sig_patterns)} patterns",
                    ""
                ])
    ...
    def _generate_fed_insights(self, fed_context: Dict, gex_data: Dict) -> List[str]:
        ...
        if days_to_fomc and days_to_fomc <= 7:
            insights.append(
                f"FOMC meeting in {days_to_fomc} days - volatility risk elevated")

        # Market stress insights
        stress_regime = fed_context['stress'].get('stress_regime', 'normal')
        if stress_regime == 'elevated':
            insights.append(
                "Elevated market stress - pattern reliability may be reduced")
        elif stress_regime == 'extreme':
            insights.append(
                "Extreme market stress - focus on high-confidence patterns only")

        # GEX regime insights
        gex_regime = gex_data.get('regime', '')
        if 'NEGATIVE_GAMMA' in gex_regime:
            insights.append(
                "Negative gamma regime - dealer hedging amplifies moves")
        elif 'POSITIVE_GAMMA' in gex_regime:
            insights.append(
                "Positive gamma regime - dealer hedging provides support")
    ...
    def _extract_pattern_names(self, df: pd.DataFrame) -> List[str]:
        ...
        pattern_columns = [col for col in df.columns if col.startswith('pattern_') or col in [
            'gamma_trap', 'gamma_flip', 'pin_risk', 'vol_squeeze', 'dealer_reload', 'liquidity_cascade'
        ]]
    ...
    def _identify_best_patterns(self, pattern_outcomes: Dict) -> List[Dict]:
        ...
        valid_patterns = [(name, data) for name, data in pattern_outcomes.items()
                          if isinstance(data, dict) and 'win_rate' in data]

        sorted_patterns = sorted(
            valid_patterns, key=lambda x: x[1]['win_rate'], reverse=True)
    ...
    def _generate_strategy_recommendations(self, high_conviction: List[Dict]) -> List[str]:
        ...
        for pattern, setups in pattern_types.items():
            ...
            recommendations.append(
                f"Focus on {pattern.upper()} setups (avg {avg_win_rate*100:.1f}% win rate)"
            )
