...
class BaselineGEXStrategy:
    ...
    def __init__(self, config_path: Optional[str] = None):
        ...
        self.conservative_position_pct = position_config.get(
            'conservative_position_pct', 1.5) / 100
        self.kelly_optimal_pct = position_config.get(
            'kelly_optimal_pct', 7.1) / 100
        self.max_position_pct = position_config.get(
            'max_position_pct', 10.0) / 100
        ...
        self.profit_target_pct = risk_config.get(
            'profit_target_pct', 1.5) / 100
        self.max_holding_days = risk_config.get('max_holding_days', 2)
        self.daily_loss_limit_pct = risk_config.get(
            'daily_loss_limit_pct', 2.0) / 100
        ...
        self.max_flip_distance = gamma_trap_config.get(
            'max_flip_distance', 5.0) / 100
        self.required_gex_regime = gamma_trap_config.get(
            'required_gex_regime', 'negative')
        self.fomc_avoidance_days = gamma_trap_config.get(
            'fomc_avoidance_days', 7)
        ...
        logger.info(
            f"  Risk: {self.stop_loss_pct:.1%}, Reward: {self.profit_target_pct:.1%}")
        logger.info(
            f"  Position Size: {self.position_size:.1%} (conservative)")
    ...
    def generate_signals(self,
                         ...
                         flip_points: Optional[Dict] = None) -> List[Dict]:
        """
            ...
            if gex_value < 0:
                ...
                if flip_points and date_str in flip_points:
                    ...
                    if price_data is not None and not price_data.empty:
                        date_price = price_data[price_data['date'] == pd.to_datetime(
                            date_str)]
        ...
        logger.info(
            f"Generated {len(signals)} baseline signals from {len(gex_data)} days")
        logger.info(
            f"  Negative GEX days: {len(signals)} ({len(signals)/len(gex_data)*100:.1f}%)")
    ...
    def _calculate_value_added(self, baseline: Dict, llm: Dict) -> Dict:
        ...
        if baseline_ev != 0:
            ev_improvement_pct = (
                (llm_ev - baseline_ev) / abs(baseline_ev)) * 100
        else:
        ...
        return {
            ...
            'selectivity_value': 'Positive' if llm_ev > baseline_ev and llm.get('total_trades', 0) < len(self.signals_generated) else 'Negative'
        }
    ...
    def _generate_conclusion(self, baseline: Dict, llm: Dict) -> str:
        ...
        if llm_ev > baseline_ev:
            improvement = ((llm_ev - baseline_ev) / abs(baseline_ev)
