    ...
    def evaluate_gamma_trap_contrarian(self, gex_data: Dict, market_data: Dict,
        ...
        if pattern_confidence >= 75:
            conditions_met.append(
                f'Confidence: {pattern_confidence:.1f}% ≥ 75%')
        else:
            conditions_failed.append(
                f'Low confidence: {pattern_confidence:.1f}% < 85%')
        ...
        if flip_point > 0 and spot_price > 0:
            ...
            if distance_to_flip < 0.05:  # Within 5%
                conditions_met.append(
                    f'Near flip: {distance_to_flip:.1%} from ${flip_point:.2f}')
            else:
                conditions_failed.append(
                    f'Far from flip: {distance_to_flip:.1%}')
        else:
    ...
    for pattern in pattern_results:
        if pattern.get('pattern') == 'gamma_trap' and pattern.get('signal') == 'CONTRARIAN':
            ...
            decision = engine.evaluate_gamma_trap_contrarian(
                gex_data, market_data, fed_context, pattern.get(
                    'confidence', 0)
            )
            ...
            if decision['action'] == 'CONTRARIAN_TRADE':
                ...
                # Add risk parameters
                trading_signal['risk_assessment'] = engine.get_risk_parameters(
                    'gamma_trap_contrarian')

                # Add statistical validation
                trading_signal['statistical_validation'] = engine.validate_rule_performance(
                    'gamma_trap_contrarian')
