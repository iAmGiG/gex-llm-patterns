...
class StatisticalValidator:
    ...
    def __init__(self, confidence_level: float = 0.95, min_samples: int = 30):
        ...
        logger.info(
            f"StatisticalValidator initialized (α={self.alpha}, min_n={min_samples})")
    ...
    def calculate_significance(self, pattern_results: Dict) -> Dict:
        ...
        for pattern, data in pattern_results.items():
            ...
            significance_results = self._perform_comprehensive_testing(
                returns, pattern)
    ...
    def bootstrap_ci(self, returns: np.ndarray,
        ...
        for _ in range(n_bootstrap):
            bootstrap_sample = np.random.choice(
                returns, size=n_samples, replace=True)
    ...
    def permutation_test(self, returns: np.ndarray,
                         null_mean: float = 0.0,
        ...
        p_value = np.sum(np.array(permutation_stats) >=
                         observed_stat) / n_permutations

        return p_value

    def t_test_one_sample(self, returns: np.ndarray,
    ...
    def calculate_effect_size(self, returns: np.ndarray,
                              baseline_returns: np.ndarray = None) -> Dict:
        """
        ...
        if baseline_returns is not None and len(baseline_returns) > 0:
            ...
            pooled_std = np.sqrt(((len(returns) - 1) * std_return**2 +
                                  (len(baseline_returns) - 1) * baseline_std**2) /
                                 (len(returns) + len(baseline_returns) - 2))

            if pooled_std > 0:
                effect_sizes['cohens_d_vs_baseline'] = (
                    mean_return - baseline_mean) / pooled_std
                ...
                if period1 in returns_by_period and period2 in returns_by_period:
                    ...
                    if len(returns1) >= 5 and len(returns2) >= 5:
                        try:
                            ...
                            logger.warning(
                                f"T-test failed for {period1} vs {period2}: {e}")

        # Overall stability score
        significant_differences = sum(1 for test in stability_tests.values()
                                      if test.get('significantly_different', False))
        total_comparisons = len(stability_tests)

        stability_score = 1.0 - \
            (significant_differences /
             total_comparisons) if total_comparisons > 0 else 1.0
    ...
    def _extract_returns_from_results(self, data: Dict) -> Optional[np.ndarray]:
        ...
        possible_keys = ['returns', 'forward_returns',
                         'return_data', 'outcomes']
    ...
    def _perform_comprehensive_testing(self, returns: np.ndarray, pattern_name: str) -> Dict:
        ...
        chi2_stat, chi2_p_value = self.chi_square_test(
            outcome_counts, expected_probability=0.5)
