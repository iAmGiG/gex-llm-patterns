...
class BaselineComparison:
    ...
    def calculate_baselines(self, start_date: str = None, end_date: str = None) -> Dict:
        ...
        results['simple_gex'] = self._calculate_simple_gex_strategy(
            market_data)
        ...
        results['pattern_contrarian'] = self._calculate_pattern_strategy(
    ...
    def _calculate_buy_hold(self, market_data: pd.DataFrame) -> Dict:
        ...
        market_data['daily_return'] = market_data['spot_price'].pct_change() * \
            100
        ...
        return {
            ...
            'sharpe': (daily_returns.mean() * np.sqrt(252)) / (daily_returns.std() * np.sqrt(252)) if daily_returns.std() > 0 else 0,
    ...
    def _calculate_simple_gex_strategy(self, market_data: pd.DataFrame) -> Dict:
        ...
        if trades:
            ...
            return {
                ...
                'volatility': trades_array.std() * np.sqrt(len(trades_array) * 252 / len(market_data)) if len(trades_array) > 1 else 0,
                'sharpe': (trades_array.mean() * np.sqrt(252)) / (trades_array.std() * np.sqrt(252)) if trades_array.std() > 0 else 0,
    ...
    def _calculate_random_baseline(self, market_data: pd.DataFrame) -> Dict:
        ...
        for sim in range(100):  # 100 simulations
            ...
            random_indices = np.random.choice(
                len(market_data) - 1, n_random_trades, replace=False)
        ...
        if simulation_results:
            avg_total_return = np.mean(
                [r['total_return'] for r in simulation_results])
            avg_win_rate = np.mean([r['win_rate'] for r in simulation_results])
            avg_trade_return = np.mean([r['avg_trade']
    ...
    def _calculate_always_short(self, market_data: pd.DataFrame) -> Dict:
        ...
        for i in range(len(market_data) - 1):
            entry_price = market_data.iloc[i]['spot_price']
            exit_price = market_data.iloc[i + 1]['spot_price']
            daily_return = (entry_price - exit_price) / \
                entry_price * 100  # Inverse for short
            daily_returns.append(daily_return)
    ...
    def _print_comparison_results(self, results: Dict):
        ...
        sorted_results = sorted(results.items(), key=lambda x: x[1].get(
            'total_return', 0), reverse=True)
        ...
        if 'pattern_contrarian' in results:
            ...
            if pattern_result.get('trades', 0) > 0:
                ...
                if pattern_result['total_return'] > bh_return:
                    print(
                        f"   ✅ Outperformed Buy & Hold: {pattern_result['total_return']:.2f}% vs {bh_return:.2f}%")
                else:
                    print(
                        f"   ⚠️ Underperformed Buy & Hold: {pattern_result['total_return']:.2f}% vs {bh_return:.2f}%")

                print(
                    f"   Win Rate: {pattern_result['win_rate']:.1f}% (target: >50%)")
                print(
                    f"   Sharpe Ratio: {pattern_result['sharpe']:.2f} (target: >1.0)")
