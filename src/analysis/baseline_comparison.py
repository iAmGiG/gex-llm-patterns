    ...
    def _print_comparison_results(self, results: Dict):
        ...
        if 'pattern_contrarian' in results:
            ...
            if pattern_result.get('trades', 0) > 0:
                ...
                print(f"   Contrarian GAMMA_TRAP outperformed random: {pattern_result['total_return']:.2f}% vs {results.get('random', {}).get('total_return', 0):.2f}%")
...
def test_baseline_comparison():
    """Test the baseline comparison system."""
    
    db_path = '.cache/consolidated_historical.db'
    comparator = BaselineComparison(db_path)
    
    print("TESTING BASELINE COMPARISON SYSTEM")
    print("=" * 60)
    
    results = comparator.calculate_baselines()
