...
class ConfidenceScorer:
    ...
    def __init__(self,
                 base_lookback_days: int = None,
                 recent_weight_days: int = None,
        ...
        self.base_lookback_days = base_lookback_days or config.get(
            'analysis.confidence_scorer.base_lookback_days', 252)
        self.recent_weight_days = recent_weight_days or config.get(
            'analysis.confidence_scorer.recent_weight_days', 63)
        self.min_sample_size = min_sample_size or config.get(
            'analysis.confidence_scorer.min_sample_size', 20)

        # Load scoring thresholds from config
        base_scores = config.get('analysis.confidence_scorer.base_scores', {})
        self.significant_base_score = base_scores.get(
            'statistically_significant', 75)
        self.not_significant_base_score = base_scores.get(
            'not_significant', 25)
        ...
        self.extreme_damping_threshold = adjustments.get(
            'extreme_damping_threshold', 90)
        self.extreme_damping_factor = adjustments.get(
        ...
        logger.info(
            f"ConfidenceScorer initialized with lookback={base_lookback_days} days")
    ...
    def calculate_pattern_confidence(self,
        ...
        """
        logger.info(
            f"Calculating confidence for pattern: {pattern_data.get('pattern', 'Unknown')}")
        ...
        if statistical_results:
            stat_score = self._score_statistical_significance(
                statistical_results)
        else:
        ...
        if recent_data:
            recent_score = self._score_recent_performance(
                recent_data, pattern_data)
        else:
        ...
        context_score = self._score_context_factors(
            market_context, pattern_data)
        ...
        calibrated_confidence = self._calibrate_confidence(
            adjusted_confidence, pattern_data)
    ...
    def batch_score_patterns(self,
        ...
        for pattern_name, pattern_data in patterns_data.items():
            ...
            if statistical_results and 'results' in statistical_results:
                pattern_stats = statistical_results['results'].get(
                    pattern_name)
    ...
    def _score_historical_performance(self, pattern_data: Dict) -> float:
        ...
        win_rate = pattern_data.get(
            'win_rate', 50.0) / 100.0  # Convert to decimal
        ...
        if sample_size < self.min_sample_size:
            size_penalty = (self.min_sample_size - sample_size) / \
    ...
    def _score_statistical_significance(self, statistical_results: Dict) -> float:
        ...
        base_score = self.significant_base_score if statistical_results[
            'overall_significant'] else self.not_significant_base_score
    ...
    def _score_recent_performance(self, recent_data: Dict, historical_data: Dict) -> float:
        ...
        consistency_adjustment = (consistency - 0.8) * \
            50  # Reward consistency > 80%
    ...
    def _apply_confidence_adjustments(self,
        ...
        if sample_size < self.min_sample_size:
            penalty_factor = 1.0 - \
                (self.min_sample_size - sample_size) / self.min_sample_size * 0.3
            adjusted *= penalty_factor

        # Extreme confidence dampening (reduce overconfidence)
        if adjusted > self.extreme_damping_threshold:
            adjusted = self.extreme_damping_threshold + \
                (adjusted - self.extreme_damping_threshold) * \
                self.extreme_damping_factor  # Dampen extreme high confidence
        elif adjusted < 10:
    ...
    def _generate_batch_summary(self, pattern_scores: Dict) -> Dict:
        ...
        confidences = [score['overall_confidence']
                       for score in pattern_scores.values()]
        sample_sizes = [score['sample_size']
                        for score in pattern_scores.values()]
