...
class TechnicalIndicatorBaseline:
    ...
    def __init__(self, config_path: Optional[str] = None):
        ...
        self.position_size = position_config.get(
        ...
        logger.info(
            f"  RSI: period={self.rsi_period}, oversold={self.rsi_oversold}, overbought={self.rsi_overbought}")
        logger.info(
    ...
    def generate_signals(self, price_data: pd.DataFrame) -> List[Dict]:
        ...
        if price_data.empty or len(price_data) < max(self.macd_slow, self.rsi_period):
            logger.warning(f"Insufficient data for technical indicators: {len(price_data)} days, need {max(self.macd_slow, self.rsi_period)}")
            return []
        ...
        logger.info(
            f"Generated {len(signals)} technical indicator signals from {len(price_data)} days")
...
class GEXAwareTechnicalBaseline(TechnicalIndicatorBaseline):
    ...
    def generate_signals_with_gex(self,
                                  price_data: pd.DataFrame,
                                  gex_data: Dict) -> List[Dict]:
        """
        ...
        logger.info(
            f"Generated {len(filtered_signals)} GEX-aware technical signals")
        logger.info(
            f"  Filtered from {len(tech_signals)} original technical signals")
...
def run_technical_baseline_test():
    ...
    if signals:
        ...
        for signal in signals[:3]:  # Show first 3
            logger.info(f"  {signal['date']}: {signal['direction'].upper()} "
    ...
    gex_signals = gex_tech_baseline.generate_signals_with_gex(
        price_data, gex_data)
