...
class PatternProbabilityMapper:
    ...
    def __init__(self, database_path: str, min_samples: int = 10):
        """
        ...
        logger.info(f"Initialized PatternProbabilityMapper with database: {database_path}")
    ...
    def analyze_pattern_outcomes(self, 
        ...
        query = self._build_outcome_query(pattern_name, lookforward_days, confidence_threshold)
        ...
        try:
            ...
            results = {}
            patterns = df['pattern_name'].unique() if pattern_name is None else [pattern_name]
            
            for pattern in patterns:
                pattern_data = df[df['pattern_name'] == pattern]
                
                if len(pattern_data) < self.min_samples:
                    logger.info(f"Skipping {pattern}: only {len(pattern_data)} samples")
                    continue
                
                # Calculate comprehensive statistics
                outcome = self._calculate_pattern_statistics(pattern, pattern_data, lookforward_days)
    ...
    def generate_probability_report(self, output_path: str = None) -> str:
        ...
        report.append("-" * 70)
        report.append(f"{'Pattern':<25} {'Win%':<8} {'Avg Ret':<10} {'Sharpe':<8} {'Samples':<8} {'Score':<8}")
        report.append("-" * 70)
...
# These would contain the complete logic from the provided code
    ...
    def _calculate_pattern_statistics(self, pattern_name, data, lookforward_days):
        ...
        excess_return = avg_return - (self.risk_free_rate * 100 * lookforward_days)
        sharpe = (excess_return / std_return) * np.sqrt(252 / lookforward_days) if std_return > 0 else 0
        
        # Calmar ratio
        calmar = (avg_return * 252 / lookforward_days) / abs(max_drawdown) if max_drawdown != 0 else 0
...
if __name__ == "__main__":
    ...
    print("PatternProbabilityMapper - Enhanced Version")
    print("🔍 This is the core research component for pattern analysis")
    print("📊 Key features:")
    print("  - Statistical rigor with confidence intervals")
    print("  - Context analysis (FOMC, GEX regimes, stress levels)")  
    print("  - High-conviction setup identification")
    print("  - Baseline comparison framework")
    print("  - Risk-adjusted performance metrics")
    print("\n💡 Use this to answer: 'Do GEX patterns actually predict anything?'")