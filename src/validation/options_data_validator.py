...
class OptionsDataValidator:
    ...
    REQUIRED_COLUMNS = [
        ...
        'date', 'implied_volatility', 'delta', 'gamma', 'open_interest'
    ]

    OPTIONAL_COLUMNS = [
        'last', 'mark', 'bid', 'ask', 'volume',
        'bid_size', 'ask_size', 'theta', 'vega', 'rho'
    ]
    ...
    def validate(self, df):
        ...
        self.validation_report['dropped_rows'] = (
            self.validation_report['input_rows'] -
            self.validation_report['output_rows']
        )
    ...
    def _validate_dtypes(self, df):
        ...
        # Numeric columns
        numeric_cols = [
            'strike', 'implied_volatility', 'delta', 'gamma',
        ...
        if 'type' in df.columns:
            ...
            invalid_types = df[~df['type'].isin(
                ['call', 'put'])]['type'].unique()
    ...
    def _validate_greeks(self, df):
        ...
        for greek, bounds_dict in self.GREEK_BOUNDS.items():
            ...
            for opt_type in ['call', 'put']:
                ...
                # Check bounds
                out_of_bounds = (
                    (df.loc[mask, greek] < bounds[0]) |
                    (df.loc[mask, greek] > bounds[1])
                )
                ...
                if out_of_bounds.any():
                    ...
                    if self.strict_mode:
                        ...
                        # Clip to bounds
                        df.loc[mask & out_of_bounds, greek] = df.loc[
                            mask & out_of_bounds, greek
                        ].clip(bounds[0], bounds[1])
