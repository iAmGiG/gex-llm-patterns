...
class UnifiedDataSystem:
    ...
    def _store_daily_database(self, conn, date: str, symbol: str, data: Dict, data_type: str):
        ...
        if data_type == 'options':
            ...
            conn.execute("INSERT OR REPLACE INTO options_data (date, symbol, data) VALUES (?, ?, ?)",
            ...
            conn.execute("""INSERT OR REPLACE INTO market_data
                       ...
                       (date, symbol, data.get('close'), data.get('volume'),
            ...
            additional_data = {k: v for k, v in data.items()
                             if k not in ['net_gex', 'gex_regime', 'gamma_flip', 'date', 'symbol']}
            conn.execute("""INSERT OR REPLACE INTO gex_data
                       (date, symbol, net_gex, gex_regime, gamma_flip, data) VALUES (?, ?, ?, ?, ?, ?)""",
                       (date, symbol, data.get('net_gex'), data.get('gex_regime'),
                        data.get('gamma_flip'), json.dumps(additional_data) if additional_data else None))
    ...
    def get_algo_time_data(self, start_date: str, end_date: str,
                          symbol: str = "SPY",
                          algo_time: str = "15:30:00",
                          weekday: int = None) -> List[Dict]:
        """
    ...
    def _query_algo_time_database(self, start_date: str, end_date: str, symbol: str,
                                 algo_time: str, weekday: int = None) -> List[Dict]:
        """
    ...
    def get_performance_stats(self) -> Dict:
        ...
        # Overall rates
        if total > 0:
            total_hits = (self.tier_stats['daily_tier1_hits'] + self.tier_stats['daily_tier2_hits'] +
                         self.tier_stats['intraday_tier1_hits'] + self.tier_stats['intraday_tier2_hits'])
