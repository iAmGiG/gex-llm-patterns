...
Merged from utils/data_normalizer.py with existing options schemas.
"""
...
MARKET_SCHEMA = {
    ...
    "volume": "float",                  # Trading volume
    "source": "str",                    # Data source (e.g., "Polygon", "AlphaVantage")
}
...
def validate_schema(df: pd.DataFrame, schema: Dict[str, str], strict: bool = False) :
    ...
    for col in common_cols:
        ...
        if expected_type.startswith("datetime") and not pd.api.types.is_datetime64_any_dtype(df[col]):
            results["type_mismatches"].append(f"{col}: expected {expected_type}, got {actual_type}")
