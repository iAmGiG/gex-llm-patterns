...
class AlphaVantageAdapter(DataSourceAdapter):
    ...
    def parse_raw_data(self, raw_data: Any) -> pd.DataFrame:
        ...
        if isinstance(raw_data, dict) and 'data' in raw_data:
            ...
            raise ValueError(f"Unsupported Alpha Vantage data format: {type(raw_data)}")
...
class SampleDataAdapter(DataSourceAdapter):
    ...
    def parse_raw_data(self, raw_data: Any) -> pd.DataFrame:
        ...
        if isinstance(raw_data, pd.DataFrame):
            ...
            raise ValueError(f"Unsupported sample data format: {type(raw_data)}")
...
class DerivedFieldCalculator:
    ...
    def calculate_moneyness_fields(self, df: pd.DataFrame, 
        ...
        if underlying_price is None or underlying_price <= 0:
            logger.warning("Invalid underlying price, skipping moneyness calculations")
            df['moneyness'] = np.nan
    ...
    def calculate_activity_ratios(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df['vol_oi_ratio'] = np.where(
            ...
            np.where(df['volume'] > 0, np.inf, 0)  # Infinite ratio for new contracts
        )
        
        # Round to appropriate precision
        ratio_precision = self.precision_rules.get('ratios', 3)
        df['vol_oi_ratio'] = df['vol_oi_ratio'].replace([np.inf], 999.999).round(ratio_precision)
...
class QualityAssessment:
    ...
    def assess_data_completeness(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        quality_score = 0.0
        quality_score += df['has_pricing'].astype(float) * (self.weights['pricing'] * 100)
        quality_score += df['has_greeks'].astype(float) * (self.weights['greeks'] * 100)
        quality_score += df['has_volume'].astype(float) * (self.weights['volume'] * 100)
...
class OptionsDataNormalizer:
    ...
    def normalize_options_data(self, 
        ...
        try:
            ...
            mapped_data = self._apply_field_mapping(parsed_data, adapter.get_field_mapping())
            metadata['processing_steps'].append('apply_field_mapping')
            
            # Step 3: Apply source-specific processing
            processed_data = adapter.apply_source_specific_processing(mapped_data)
            ...
            if self.config.enable_derived_fields:
                enriched_data = self._calculate_derived_fields(standardized_data, underlying_price)
                metadata['processing_steps'].append('calculate_derived_fields')
            else:
                enriched_data = standardized_data
            
            # Step 7: Apply quality assessment
            quality_data = self.quality_assessor.assess_data_completeness(enriched_data)
            quality_data = self.quality_assessor.add_validation_flags(quality_data)
            ...
            metadata['avg_quality_score'] = final_data['data_quality_score'].mean() if 'data_quality_score' in final_data.columns else 0
    ...
    def _apply_field_mapping(self, df: pd.DataFrame, field_mapping: Dict[str, str]) -> pd.DataFrame:
        ...
        rename_dict = {old: new for old, new in field_mapping.items() if old in mapped_df.columns}
            ...
            if field in typed_df.columns:
                try:
                    if dtype.startswith('datetime'):
                        typed_df[field] = pd.to_datetime(typed_df[field], utc=True)
                    elif dtype.startswith('float'):
                        typed_df[field] = pd.to_numeric(typed_df[field], errors='coerce')
                    elif dtype.startswith('int'):
                        typed_df[field] = pd.to_numeric(typed_df[field], errors='coerce', downcast='integer')
                    elif dtype == 'bool':
    ...
    def _calculate_derived_fields(self, df: pd.DataFrame, underlying_price: Optional[float]) -> pd.DataFrame:
        ...
        # Pricing fields
        enriched_df = self.field_calculator.calculate_pricing_fields(enriched_df)
        
        # Time fields
        enriched_df = self.field_calculator.calculate_time_fields(enriched_df)
        
        # Activity ratios
        enriched_df = self.field_calculator.calculate_activity_ratios(enriched_df)
        
        # Moneyness fields (if underlying price available)
        if underlying_price is not None:
            enriched_df = self.field_calculator.calculate_moneyness_fields(enriched_df, underlying_price)
