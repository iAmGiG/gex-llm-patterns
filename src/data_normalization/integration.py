...
class NormalizedDataPipeline:
    ...
    def process_options_data(self,
        ...
        try:
            ...
            logger.info(f"Normalizing {symbol} options data from {source} for {trading_date}")
            ...
            if normalized_df.empty:
                result['warnings'].append("Normalization produced empty dataset")
                return result
            ...
            if self.enable_validation:
                ...
                validated_df, validation_report = self.validator.validate(validation_df)
            ...
            if cache_result and not result['data'].empty:
                try:
                    ...
                    if cache_success:
                        logger.info(f"Cached normalized data for {symbol} {trading_date} ({cache_format})")
                    else:
                        result['warnings'].append(f"Failed to cache normalized data")
                        
                except Exception as e:
                    result['cache_status'] = 'error'
                    result['warnings'].append(f"Cache error: {str(e)}")
                    logger.warning(f"Cache error for {symbol} {trading_date}: {e}")
    ...
    def process_cached_data(self,
        ...
        try:
            # Load from cache
            cached_df = self.cache_manager.get_options_data(symbol, trading_date)
            
            if cached_df is None:
                result['errors'].append(f"No cached data found for {symbol} {trading_date}")
                return result
            ...
            if reprocess_derived and underlying_price is not None:
                logger.info(f"Reprocessing derived fields with underlying price {underlying_price}")
            ...
            logger.info(f"Loaded {len(result['data'])} cached contracts for {symbol} {trading_date}")
...
def process_alpha_vantage_options(raw_data: Any,
    ...
    if not result['success']:
        logger.error(f"Failed to process Alpha Vantage data: {result['errors']}")
