...
class LiveGEXInterface:
    ...
    def calculate_gex_for_symbol(self,
        ...
        if options_data is None or options_data.empty:
            return {
                'status': 'error',
                'message': f"No options data provided for {symbol} on {trading_date}"
            }
        ...
        try:
            logger.info(
                f"Calculating GEX for {symbol} on {trading_date} with {len(options_data)} contracts")

            # Validate data if requested
            if self.validate_data:
                logger.info("Validating options data")
                cleaned_data, validation_report = self.validator.validate(
                    options_data)

                if validation_report.get('issues'):
                    logger.warning(
                        f"Data validation issues: {validation_report['issues']}")
            ...
            obfuscation_metadata = {}
            if self.obfuscate_data:
                options_data, obfuscation_metadata = self._obfuscate_options_data(
                    options_data, symbol, trading_date)
            ...
            call_gex, put_gex = self._calculate_type_gex(
            ...
            summary_stats = self._calculate_summary_stats(
                options_data, spot_price, call_gex, put_gex, net_gex
            )
            ...
            logger.info(
                f"Successfully calculated GEX: Net={net_gex:.0f}, Calls={call_gex:.0f}, Puts={put_gex:.0f}")
            return results
    ...
    def _calculate_summary_stats(self, df, spot_price, call_gex, put_gex, net_gex):
        ...
        try:
            ...
            ) if 'open_interest' in df.columns else 0
