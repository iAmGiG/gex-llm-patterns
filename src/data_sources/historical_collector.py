...
class HistoricalOptionsCollector:
        ...
        for i, trade_date in enumerate(remaining_dates):
            try:
                ...
                if cached_data is not None and not cached_data.empty:
                    self.logger.info(f"Using cached options data for {symbol} {trade_date} ({len(cached_data)} options)")
                    ...
                    if is_valid:
                        ...
                        if cache_stored:
                            ...
                            self.logger.info(f"✅ Successfully collected and cached {len(options_data)} options for {trade_date}")
                        else:
    ...
    async def collect_multi_symbol_historical(self, symbols: List[str], start_date, end_date) :
        ...
        overall_summary['collection_end'] = datetime.now().isoformat()
