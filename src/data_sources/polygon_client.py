...
class PolygonClient:
    ...
    def __init__(self, api_key: str = None):
        ...
        if not api_key:
            try:
                config_path = Path(
                    __file__).parent.parent.parent / 'config' / 'config.json'
        ...
        config = get_config()
        self.requests_per_minute = config.get(
            'data_source.polygon_client.requests_per_minute', 5)
        self.rate_limit_timeout = config.get(
            'data_source.polygon_client.rate_limit_timeout', 60)
    ...
    def _rate_limit(self):
        ...
        self.last_request_times = [
            t for t in self.last_request_times if now - t < self.rate_limit_timeout]
    ...
    def fetch_daily_bars(self, symbol, start_date, end_date):
        ...
        try:
            ...
            self.logger.info(
                f"Fetching {symbol} daily data: {start_date} to {end_date}")
            response = requests.get(url, params=params, timeout=30)

            if response.status_code == 403:
                self.logger.error(
                    "Authentication failed - check your Polygon API key")
                return None
            ...
            if data.get('status') not in ['OK', 'DELAYED']:
                self.logger.warning(
                    f"API returned unexpected status: {data.get('status')}")
                return None
            ...
            df['date'] = pd.to_datetime(df['t'], unit='ms').dt.tz_localize(
            ...
            self.logger.info(
                f"Successfully fetched {len(df)} days for {symbol}")
            return df
