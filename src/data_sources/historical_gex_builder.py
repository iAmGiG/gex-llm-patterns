...
class HistoricalGEXDatabaseBuilder:
    ...
    def get_resume_point(self, symbol) :
        ...
        try:
            with self.get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(
                    'SELECT MAX(date) FROM daily_gex_metrics WHERE symbol = ?',
                    (symbol,)
                )
        ...
        try:
            ...
            for (strike, expiration), group in options_data.groupby(['strike', 'expiration']):
                ...
                row_data = {
                    ...
                    'implied_vol': calls['implied_volatility'].iloc[0] if len(calls) > 0 else (
                        puts['implied_volatility'].iloc[0] if len(puts) > 0 else 0.2
                    ),
    ...
    def calculate_daily_gex_profile(self, symbol, date, options_data: pd.DataFrame,
                                    spot_price) :
        """
    ...
    def build_gex_database(self, symbols: List[str], start_date, end_date,
        ...
        try:
            ...
            for symbol in symbols:
                ...
                for i, trade_date in enumerate(trading_dates):
                    try:
                        ...
                        gex_profile = self.calculate_daily_gex_profile(
                            symbol, trade_date, options_data, spot_price)
                        ...
                        self.logger.info(f"✅ {symbol} {trade_date}: GEX=${gex_profile['total_gex']:,.0f}, "
                ...
                symbol_summary['avg_quality_score'] = (
            ...
            summary['build_duration_minutes'] = calculate_duration_minutes(
                summary['build_start'], summary['build_end']
            )
...
if __name__ == "__main__":
    ...
    builder = HistoricalGEXDatabaseBuilder(
        database_path=".cache/enhanced_gex_database.db"
    )
