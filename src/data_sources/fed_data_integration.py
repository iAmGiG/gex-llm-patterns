...
class FedDataIntegration:
    ...
    def fetch_fomc_calendar(self, start_year: int = 2010, end_year: int = 2025) -> pd.DataFrame:
        ...
        # Create DataFrame
        df = pd.DataFrame(all_meetings, columns=[
                          'date', 'decision', 'rate', 'rate_change'])
    ...
    def calculate_market_stress(self, date: pd.Timestamp,
        ...
        if 'BAMLH0A0HYM2' in day_data:
            ...
            if not pd.isna(hy_spread):
                ...
                stress_metrics['credit_stress'] = (
                    'low' if hy_spread < 400 else
                    'normal' if hy_spread < 600 else
                    'elevated' if hy_spread < 800 else
                    'high'
                )
        ...
        if 'credit_spread' in stress_metrics:
            ...
            spread_score = min(
                100, (stress_metrics['credit_spread'] / 1000) * 100)
        ...
        if weights > 0:
            ...
            stress_metrics['stress_regime'] = (
                'calm' if stress_metrics['composite_stress'] < 25 else
                'normal' if stress_metrics['composite_stress'] < 50 else
                'elevated' if stress_metrics['composite_stress'] < 75 else
                'extreme'
            )
    ...
    def get_full_context(self, date: pd.Timestamp):
        ...
        full_context = {
            ...
            'pattern_weight_adjustments': self._calculate_pattern_weights(
                fomc_context, stress_metrics
            )
        }
    ...
    def prepare_backtest_context(self,
                                 start_date,
        ...
        """
        logger.info(
            f"Preparing Fed context for backtest: {start_date} to {end_date}")
