...
class GEXTokenizer:
    ...
    def __init__(self, 
        ...
        Args:
            lookback_days: Days to look back for percentile calculation (default 252 = 1 year)
            update_frequency: How often to update percentiles ('daily', 'weekly', 'monthly')
        """
    ...
    def tokenize_with_context(self, 
        ...
        Returnsionary with multiple tokens including context
        """
        result = {
    ...
    def get_token_statistics(self, tokens) :
        ...
        Args:
            tokens of GEX tokens
        
        Returnsionary with token frequency statistics
        """
        total = len(tokens)
        if total == 0:
            return {}
        ...
        if len(original_series) > 0:
            ...
            if extreme_high_idx.any():
                ...
                validation['extreme_high_mapping'] = (
                    sum(1 for t in high_tokens if 'EXTREME_POS' in t) / len(high_tokens)
                )
            
            if extreme_low_idx.any():
                low_tokens = [tokens[i] for i in extreme_low_idx[extreme_low_idx].index]
                validation['extreme_low_mapping'] = (
                    sum(1 for t in low_tokens if 'EXTREME_NEG' in t) / len(low_tokens)
                )
