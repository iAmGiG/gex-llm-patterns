    ...
    def get_database_info(self, db_path):
        ...
        try:
            ...
            if 'daily_gex_metrics' in tables:
                cursor.execute(
                    "SELECT MIN(date), MAX(date) FROM daily_gex_metrics;"
                )
...
Examples:
  ...
  python tools/database/backup_database.py --list
    ...
    parser.add_argument(
        '--database',
        default='gex_database.db',
        help='Database file to backup (default: gex_database.db)'
    )

    parser.add_argument(
        '--all',
        action='store_true',
        help='Backup all database files in .cache/'
    )
    ...
    try:
        if args.all:
            ...
            backup_path = backup_tool.backup_database(
                args.database,
