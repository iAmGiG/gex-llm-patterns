...
class GEXCodeReviewer:
    ...
    def __init__(self, project_root: str = "."):
        ...
        self.complex_typing_patterns = [
            r'(\w+):\s*Optional\[[^\]]+\]\s*(=)',  # param: Optional[Type] = default
            r'(\w+):\s*Union\[[^\]]+\]\s*(=)',     # param: Union[Type1, Type2] = default  
            r'(\w+):\s*List\[[^\]]*\]\s*(=)',      # param: List[Type] = default
            r'(\w+):\s*Dict\[[^\]]*\]\s*(=)',      # param: Dict[K, V] = default
            r'(\w+):\s*Set\[[^\]]*\]\s*(=)',       # param: Set[Type] = default
            r'(\w+):\s*Tuple\[[^\]]*\]\s*(=)',     # param: Tuple[Type, ...] = default
        ]
