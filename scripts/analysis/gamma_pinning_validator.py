...
class GammaPinningValidator:
    ...
    def validate_friday_gamma_pinning(self, start_date: str, end_date: str,
                                      target_time: str = "15:30:00") -> Dict:
        """
    ...
    def _analyze_friday(self, friday_date: str, target_time: str) -> Optional[Dict]:
        ...
        try:
            ...
            market_data = self.data_system.fetch_market_data(
                timestamp, self.symbol)
            if not market_data:
                logger.debug(
                    f"No market data for {friday_date} at {target_time}")
                return None
            ...
            if not max_gamma_info:
                logger.debug(
                    f"No gamma strike data for {friday_date} at {target_time}")
                return None
            ...
            movement_analysis = self._analyze_price_movement(
                friday_date, target_time, max_gamma_strike)
            ...
            logger.debug(
                f"Analyzed {friday_date}: {proximity_category}, distance=${distance_to_gamma:.2f}")
            return result
    ...
    def _calculate_summary_stats(self, results: List[Dict]) -> Dict:
        ...
        for category in ['CLOSE', 'MODERATE', 'FAR']:
            proximity_counts[category] = sum(
                1 for r in results if r['proximity_category'] == category)

        # Movement analysis
        movement_results = [
            r for r in results if r['movement_analysis']['has_movement_data']]
        moved_toward_count = sum(
            1 for r in movement_results if r['movement_analysis']['moved_toward_gamma'])

        # Distance statistics
        distances = [r['distance_to_gamma'] for r in results]
        distance_changes = [r['movement_analysis']['distance_change_pct']
        ...
        summary = {
            ...
            'movement_analysis': {
                ...
                'moved_toward_gamma_rate_pct': (moved_toward_count / len(movement_results)) * 100 if movement_results else 0
            },
    ...
    def validate_multiple_times(self, start_date: str, end_date: str,
                                times: List[str] = None) -> Dict:
        """Validate gamma pinning at multiple algo times."""
        if times is None:
            times = ['10:00:00', '14:30:00',
                     '15:30:00', '15:40:00', '15:50:00']

        results_by_time = {}

        for time_str in times:
            logger.info(f"Validating at {time_str}...")
            time_results = self.validate_friday_gamma_pinning(
                start_date, end_date, time_str)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Validate Friday gamma pinning behavior")
    parser.add_argument("--symbol", default="SPY", help="Symbol to analyze")
    parser.add_argument("--start-date", required=True,
                        help="Start date (YYYY-MM-DD)")
    parser.add_argument("--end-date", required=True,
                        help="End date (YYYY-MM-DD)")
    parser.add_argument("--time", default="15:30:00",
                        help="Target time (HH:MM:SS)")
    parser.add_argument("--multi-time", action="store_true",
                        help="Analyze multiple times")
    ...
    if args.multi_time:
        results = validator.validate_multiple_times(
            args.start_date, args.end_date)
    else:
        results = validator.validate_friday_gamma_pinning(
            args.start_date, args.end_date, args.time
        )
    ...
    if 'summary_statistics' in results:
        ...
        print(
            f"Close pins (<$5): {stats['close_pin_count']} ({stats['close_pin_rate_pct']:.1f}%)")
        ...
        for time_str, time_results in results['results_by_time'].items():
            if 'summary_statistics' in time_results:
                ...
                print(
                    f"  {time_str}: {stats['close_pin_rate_pct']:.1f}% close pin rate")
