...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Compare LLM pattern strategy vs baseline strategies"
    )
    parser.add_argument(
        "--db-path",
        type=str,
    ...
    try:
        ...
        # Run comparison
        results = comparison.calculate_baselines(
        ...
        if pattern_result.get('trades', 0) > 0:
            print()
            print("🎯 KEY FINDING:")
            print(f"  LLM-filtered strategy: {pattern_result['total_return']:.2f}% "
                  f"({pattern_result['trades']} trades, {pattern_result['win_rate']:.1f}% win rate)")
            print(f"  Simple GEX strategy: {simple_gex_result.get('total_return', 0):.2f}% "
