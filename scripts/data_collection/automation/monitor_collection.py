...
def monitor_collection():
    ...
    while True:
        try:
            ...
            total_options_dates = sum(len(dates)
                                      for dates in options_collected.values())

            print(f"Symbols with data: {len(options_collected)}")
            print(f"Total date-symbol pairs: {total_options_dates}")
            print(
                f"API calls today: {progress.get('options_calls_today', 0)}/25")
            print(
                f"Calls remaining: {25 - progress.get('options_calls_today', 0)}")
            ...
            if options_collected:
                ...
                for symbol, dates in list(options_collected.items())[:5]:
                    ...
                    print(
                        f"  {symbol}: {len(dates)} days ({earliest} to {latest})")
                if len(options_collected) > 5:
                    print(
                        f"  ... and {len(options_collected) - 5} more symbols")
            print()
            ...
            if options_summary['tickers']:
                ...
                for ticker, info in options_summary['tickers'].items():
                    ...
                    if info['dates']:
                        print(
                            f"  Date range: {min(info['dates'])} to {max(info['dates'])}")
            ...
            if total_options_dates > 0:
                ...
                print(
                    f"Options: ~{days_to_1_year:.1f} days to complete 1 year of data")
