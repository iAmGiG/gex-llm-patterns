#!/usr/bin/env python3
"""Parallel Collection for Expanded Ticker Set.

Collects historical options data for multiple tickers simultaneously,
sharing the 900 calls/min quota across symbols for faster collection.

This is much faster than sequential collection because API calls are
interleaved across tickers instead of waiting for one symbol to complete.

Usage:
    # Start parallel collection for expanded tickers (15 symbols)
...
def setup_logging(verbose: bool = False):
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler(project_root / "parallel_collection.log"),
...
def estimate_collection(symbols: list, start_date: str, end_date: str) -> dict:
    """Estimate collection time for parallel mode."""
    ...
    # Parallel: rate limit applies to all symbols together, not per symbol
    # So time is total_calls / 900, not (trading_days * len(symbols)) / 900
    ...
    return {
        ...
        "speedup": f"{len(symbols)}x faster than sequential",
    }


async def run_parallel_collection(args):
    ...
    print(f"\n{'='*70}")
    print("PARALLEL HISTORICAL OPTIONS DATA COLLECTION")
    print(f"{'='*70}")
    ...
    if not args.yes:
        response = input("Start parallel collection? [y/N]: ")
        if response.lower() != "y":
            print("Aborted.")
            return
    ...
    summary = await collector.collect_multi_symbol_historical(
        ...
        parallel=True,  # Use parallel mode
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Collect historical options data in parallel (interleaved API calls)",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Expanded multi-asset collection (15 symbols)
  %(prog)s -y
  ...
  %(prog)s --status
    ...
    asyncio.run(run_parallel_collection(args))
