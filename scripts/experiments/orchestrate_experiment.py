...
def run_batch_validation(args):
    ...
    experiment_template = args.experiment_template or (
        "Analyze {symbol} patterns on {date} at {time}. "
        "Focus on gamma dynamics and dealer positioning."
    )
    ...
    if not batch_mode or len(args.batch_dates) == 1:
        ...
        for date in args.batch_dates:
            ...
            experiment_desc = experiment_template.format(
                symbol=args.symbol,
                date=date,
                time=args.time_window
            )
...
def main():
    ...
    parser.add_argument("--batch-dates", type=str, nargs='+',
                        help="Run batch validation on multiple dates")
    parser.add_argument("--time-window", type=str, default="15:30",
                        help="Time window for analysis (default: 15:30)")
    parser.add_argument("--confidence-threshold", type=int, default=60,
                        help="Minimum confidence for signal counting (default: 60)")
    parser.add_argument("--target-signals", type=int, default=3,
                        help="Target number of signals for validation pass (default: 3)")
    parser.add_argument("--experiment-template", type=str,
    ...
    parser.add_argument("--no-batch-mode", action="store_true",
                        help="Disable batch mode, process dates individually")
