...
def main():
    ...
    results.append(test_specification(
        data, 'diff', 'level', 5,
        "Original: Differenced GEX vs Level Vol (Lags 1-5)"
    ))

    # Spec 2: Both in levels
    results.append(test_specification(
        data, 'level', 'level', 5,
        "Both Levels (Lags 1-5)"
    ))

    # Spec 3: Both differenced
    results.append(test_specification(
        data, 'diff', 'diff', 5,
        "Both Differenced (Lags 1-5)"
    ))

    # Spec 4: Extended lags
    results.append(test_specification(
        data, 'diff', 'level', 10,
        "Extended Lags: Differenced GEX vs Level Vol (Lags 1-10)"
    ))

    # Spec 5: Percentage changes
    results.append(test_specification(
        data, 'pct_change', 'pct_change', 5,
        "Percentage Changes (Lags 1-5)"
    ))

    # Spec 6: Short lags only (1-3)
    results.append(test_specification(
        data, 'diff', 'level', 3,
        "Short Lags Only: Differenced GEX vs Level Vol (Lags 1-3)"
    ))
