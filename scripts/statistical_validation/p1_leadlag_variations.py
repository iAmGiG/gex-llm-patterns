...
def load_data():
    ...
    data = data[[
        'date', 'net_gex', 'spot_price', 'forward_return_t1', 'realized_vol_t1'
    ]].dropna()
...
def test_terciles(data):
    ...
    data['gex_tercile'] = pd.qcut(
        data['net_gex'],
        q=3,
        labels=['Most Negative', 'Medium Negative', 'Least Negative']
    )

    # Calculate statistics by tercile
    tercile_stats = data.groupby('gex_tercile').agg({
        'realized_vol_t1': ['mean', 'std', 'count'],
