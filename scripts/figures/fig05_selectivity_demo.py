...
def create_figure(windows, output_path):
    ...
    for ax, key in zip(axes_flat, window_keys):
        ...
        colors = [DARK_THEME['accent_positive'] if v > 0 else DARK_THEME['accent_negative']
                  for v in gex]

        # Plot bars
        days = np.arange(1, 31)
        bars = ax.bar(days, gex, color=colors, width=0.8,
                      edgecolor=DARK_THEME['background'], linewidth=0.3, alpha=0.9)

        # Add zero line
        ax.axhline(0, color=DARK_THEME['dim'], linestyle='-', linewidth=1)

        # Add $5B threshold lines
        ax.axhline(-5, color=DARK_THEME['accent_warning'], linestyle='--',
        ...
        ax.set_title(f"{window['label']}\n[{window['status']}]",
                     fontsize=12, fontweight='bold', color=badge_color, pad=10)
    ...
    fig.suptitle('Framework Selectivity Demonstration\n'
                 ...
                 fontsize=14, fontweight='bold', color=DARK_THEME['text'], y=0.98)

    # Criteria legend at bottom
    criteria_text = (
        "Detection Criteria: Persistence > 70% same sign  |  "
        "Avg Magnitude > $5B  |  ≤5 Sign Flips"
    )
    fig.text(0.5, 0.02, criteria_text, ha='center', va='bottom', fontsize=10,
