...
def create_figure(data, output_path):
    ...
    ax.scatter(data['persistence'][rejected_mask], data['confidence'][rejected_mask],
               s=sizes_rejected, c=DARK_THEME['accent_negative'], alpha=0.5,
    ...
    ax.scatter(data['persistence'][detected_mask], data['confidence'][detected_mask],
               s=sizes_detected, c=DARK_THEME['accent_positive'], alpha=0.7,
               label=f'Detected (n={n_detected}, mean={mean_conf_detected:.1f}%)',
               edgecolors=DARK_THEME['background'], linewidths=0.5)

    # Add persistence threshold line at 70%
    ax.axvline(70, color=DARK_THEME['accent_neutral'], linestyle='--', linewidth=2.5,
               label='70% Persistence Threshold', zorder=10)

    # Add mean confidence lines
    ax.axhline(mean_conf_detected, color=DARK_THEME['accent_positive'], linestyle=':',
               linewidth=2, alpha=0.8)
    ax.axhline(mean_conf_rejected, color=DARK_THEME['accent_negative'], linestyle=':',
               linewidth=2, alpha=0.8)

    # Annotate confidence gap
    gap_x = 95
    ax.annotate('', xy=(gap_x, mean_conf_detected), xytext=(gap_x, mean_conf_rejected),
                arrowprops=dict(arrowstyle='<->', color=DARK_THEME['accent_warning'],
                               lw=2.5, shrinkA=0, shrinkB=0))
    ax.text(gap_x + 1.5, (mean_conf_detected + mean_conf_rejected) / 2,
            f'{conf_gap:.1f}pp\ngap',
            fontsize=12, fontweight='bold', color=DARK_THEME['accent_warning'],
            va='center', ha='left')
    ...
    ax.text(0.02, 0.98, stats_text, transform=ax.transAxes, fontsize=11,
            verticalalignment='top', color=DARK_THEME['text'],
    ...
    ax.set_title('LLM Confidence Discrimination Across Full Detection Spectrum\n'
