...
def verify_phase1_data():
    ...
    for pattern, expected_count in expected.items():
        actual = criteria_df[(criteria_df['pattern_type'] == pattern) &
                            (criteria_df['detected'] == True)].shape[0]
    ...
    for criterion in ['criterion_1_volatility_amplification',
...
def create_summary_statistics():
    ...
    for i, criterion in enumerate(['criterion_1_volatility_amplification',
                                   'criterion_2_directional_followthrough',
                                   'criterion_3_strike_convergence',
                                   'criterion_4_range_expansion'], 1):
        rate = det_df[criterion].sum() / len(det_df) * 100
        summary['phase1_materialization_rates'][f'C{i}'] = f"{rate:.1f}%"

    # Phase 2: Baseline comparison
    for i, criterion in enumerate([('criterion_1_volatility_amplification', 'C1'),
                                   ('criterion_4_range_expansion', 'C4')]):
