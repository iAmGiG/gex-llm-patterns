...
def recalculate_c3_with_flip_points(df, flip_points_df):
    ...
    df = df.merge(flip_points_df[['date', 'spot_price', 'flip_point', 'distance_from_spot']],
                  on='date', how='left', suffixes=('', '_merged'))
...
def build_contingency_matrix(df):
    ...
    matrix_df = pd.DataFrame(
        matrix,
        index=patterns,
        columns=['C1: Vol Amp', 'C2: Directional', 'C3: Convergence', 'C4: Range Exp']
    )
...
def main():
    ...
    specificity_results = analyze_mechanism_specificity(
        matrix,
