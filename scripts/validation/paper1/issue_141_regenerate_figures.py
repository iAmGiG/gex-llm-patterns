...
def load_data():
    ...
    if not data_path.exists():
        ...
        df['gex_concentration'] = df['detected'].apply(
            lambda x: np.random.normal(detected_mean, 0.020) if x
            else np.random.normal(not_detected_mean, 0.017)
        )
    else:
...
def figure1_gex_concentration(df, output_path):
    ...
    ax.set_title('GEX Concentration Distribution\n(Detection vs Non-Detection Days)',
...
def figure2_detection_calendar(df, output_path):
    ...
    # REMOVED "Issue #141" from title
    fig.suptitle('Detection Status Calendar 2024',
