...
def main():
    ...
    gex_concentration_df = pd.DataFrame({
        'date': gex_concentration.index,
        'gex_concentration': gex_concentration.values
    })
    ...
    df = df.merge(daily_df[['date', 'net_call_gex', 'net_put_gex', 'put_call_ratio',
                             'data_quality_score', 'options_count']], on='date', how='left')
    ...
    results.append(test_hypothesis(
        abs(detected['net_gex']),
        abs(not_detected['net_gex']),
        'H1: GEX Magnitude (Signal Strength)'
    ))

    # H2: Realized Volatility (signal noise)
    print("  H2: Realized Volatility...")
    results.append(test_hypothesis(
        detected['realized_vol_t1'],
        not_detected['realized_vol_t1'],
        'H2: Realized Volatility T+1 (Signal Noise)'
    ))
    ...
    results.append(test_hypothesis(
        detected['options_count'],
        not_detected['options_count'],
        'H7B: Options Count (Market Depth)'
    ))
    ...
    print("=" * 80)
    print()
    print(results_df[['hypothesis', 'detected_mean', 'not_detected_mean',
                       'p_value', 'cohen_d', 'effect_size', 'significant']].to_string(index=False))
    print()
