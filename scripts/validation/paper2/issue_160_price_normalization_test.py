...
class PriceNormalizationTester:
    ...
    def run_experiment(self) -> Dict:
        ...
        print(f"Original Detection Rate: {original_detection_rate*100:.1f}% ({sum(r['detected'] for r in original_results)}/{len(original_results)} windows)")
        ...
        print(f"Normalized Detection Rate: {normalized_detection_rate*100:.1f}% ({sum(r['detected'] for r in normalized_results)}/{len(normalized_results)} windows)")
        ...
        print(f"  Persistence (≥70%):     {original_persistence}/{len(original_results)} → {normalized_persistence}/{len(normalized_results)} ({normalized_persistence-original_persistence:+d})")
        print(f"  Magnitude (≥$5B):       {original_magnitude}/{len(original_results)} → {normalized_magnitude}/{len(normalized_results)} ({normalized_magnitude-original_magnitude:+d})")
        print(f"  Stability (≤5 flips):   {original_stability}/{len(original_results)} → {normalized_stability}/{len(normalized_results)} ({normalized_stability-original_stability:+d})")
...
\hline
Detection Rate & """ + f"{results['original_detection_rate']*100:.1f}\\%" + r""" & """ + f"{results['normalized_detection_rate']*100:.1f}\\%" + r""" \\
Scaling Factor & 1.00x & """ + f"{results['scaling_factor']:.2f}x" + r""" \\
\hline
\multicolumn{3}{l}{\textit{Criterion Pass Rates (out of 223 windows)}} \\
\hline
Persistence ($\geq$70\%) & """ + f"{results['criteria_analysis']['original']['persistence']}" + r""" & """ + f"{results['criteria_analysis']['normalized']['persistence']}" + r""" \\
Magnitude ($\geq$\$5B) & """ + f"{results['criteria_analysis']['original']['magnitude']}" + r""" & """ + f"{results['criteria_analysis']['normalized']['magnitude']}" + r""" \\
Stability ($\leq$5 flips) & """ + f"{results['criteria_analysis']['original']['stability']}" + r""" & """ + f"{results['criteria_analysis']['normalized']['stability']}" + r""" \\
\hline
\multicolumn{3}{l}{\textbf{""" + f"Conclusion: {results['conclusion'].replace('_', ' ').title()}" + r"""}} \\
\hline
\end{tabular}
\end{table}
"""
...
def main():
    ...
    try:
        ...
        tester.generate_latex_table(
            results,
            output_dir / "issue_160_price_normalization_table.tex"
        )

        tester.generate_csv_output(
            results,
            output_dir / "issue_160_price_normalization_results.csv"
        )
