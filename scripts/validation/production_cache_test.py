...
class ProductionCacheTest:
    ...
    def test_production_flow(self, symbol: str, date: str) -> dict:
        ...
        if not is_valid_trading_date(date):
            return {
                'status': 'error',
                'message': f'Invalid trading date: {date}'
            }
        ...
        try:
            ...
            if options_result.get('status') == 'success':
                ...
                logger.info(
                    f"✅ Options data: {len(data)} contracts from {options_result.get('source', 'unknown')}")
            else:
                results['data_flow_test'] = {
                    'status': 'error',
                ...
                logger.error(
                    f"❌ Options data failed: {results['data_flow_test']['message']}")
                return results
        except Exception as e:
            results['data_flow_test'] = {
                'status': 'error',
                'message': f'Options data exception: {str(e)}'
            }
        ...
        try:
            ...
            if gex_result.get('status') == 'success':
                ...
                logger.info(
                    f"✅ GEX: ${metrics.get('total_gex', 0):,.0f} at ${metrics.get('spot_price', 0):.2f}")
            else:
                results['gex_calculation'] = {
                    'status': 'error',
                ...
                logger.error(
                    f"❌ GEX calculation failed: {results['gex_calculation']['message']}")
                return results
        except Exception as e:
            results['gex_calculation'] = {
                'status': 'error',
                'message': f'GEX calculation exception: {str(e)}'
            }
        ...
        try:
            ...
            if llm_response:
                ...
                if isinstance(llm_response, dict):
                    ...
                    if mechanics.get('error') or mechanics.get('who') == 'Error':
                        ...
                        error_msg = mechanics.get(
                            'narrative', 'Unknown LLM error')
                        logger.error(f"❌ LLM analysis failed: {error_msg}")

                        # Check if it's a token limit error
                        if 'max_tokens' in error_msg:
                            logger.info(
                                "🔄 Retrying LLM analysis with higher token limit...")
                        ...
                        logger.error(
                            "❌ LLM analysis failed - marking test as invalid")
                        return results
                ...
                reasoning = getattr(
                    llm_response, 'reasoning', str(llm_response))
                ...
                logger.info(
                    f"✅ LLM: {confidence}% confidence, direction: {direction}")
            else:
                results['llm_analysis'] = {
                    'status': 'error',
            ...
            results['llm_analysis'] = {
                'status': 'error',
                'message': f'LLM analysis exception: {str(e)}'
            }
        ...
        try:
            ...
            results['pattern_detection'] = {
                'status': 'error',
                'message': f'Pattern detection exception: {str(e)}'
            }
        ...
        success_count = sum(
            1 for test in test_components if test.get('status') == 'success')
        failed_tests = [
            test for test in test_components if test.get('status') == 'error']

        # Check for critical failures (LLM token limits, etc.)
        critical_failures = [
            test for test in failed_tests if test.get('retry_needed')]

        if critical_failures:
            results['overall_status'] = 'failed_retry_needed'
            logger.error(
                "❌ Production test FAILED: Critical errors require retry")
            for failure in critical_failures:
                logger.error(
                    f"  - {failure.get('error_type', 'unknown')}: {failure.get('message', 'unknown error')}")
            ...
            logger.warning(
                f"⚠️ Production test PARTIAL: {success_count}/4 components working")
        else:
            results['overall_status'] = 'failed'
            logger.error(
                f"❌ Production test FAILED: Only {success_count}/4 components working")
    ...
    def run_production_test(self, symbol: str, date: str) -> str:
        ...
        filepath = reports_manager.save_agent_results(
            agent_name="ProductionCacheTest",
        ...
        for component, data in results.items():
            if isinstance(data, dict) and 'status' in data:
                ...
                logger.info(
                    f"{component.replace('_', ' ').title()}: {status} - {message}")
        ...
        if results['pattern_detection'].get('status') == 'success':
            ...
            logger.info(
                f"  • Is Friday: {'✅' if patterns.get('is_friday') else '❌'}")
            logger.info(
                f"  • Pattern Signals: {patterns.get('signal_count', 0)}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--symbol", type=str,
                        default="SPY", help="Trading symbol")
